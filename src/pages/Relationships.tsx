  ...
  return (
    <div className={embedded ? undefined : 'app'}>
          ...
          <div className="actions-row">
            {characters.length > 0 && (
              <button className="button button-ghost" onClick={() => setMapView((v) => !v)}>
                <Icon
                  icon={mapView ? 'fa-solid fa-heart' : 'fa-solid fa-diagram-project'}
                  style={{ marginRight: 6 }}
                />{' '}
                {mapView ? 'List view' : 'Constellation map'}
              </button>
            )}
            <button
            ...
            {relationships.map((r) => (
              <div className="rel-item" key={r.id} onContextMenu={(e) => relMenu(e, r)}>
                <div className="rel-card-head">
                  <span className="rel-card-kicker">
                    <Icon icon="fa-solid fa-link" /> STORY THREAD
                  </span>
                  <span className="rel-card-index">
                    {String(relationships.indexOf(r) + 1).padStart(2, '0')}
                  </span>
                </div>
                <div className="names">
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                    <span className="rel-avatar" style={{ background: charColor(r.a) }}>
                      {initials(charName(r.a))}
                    </span>
                    <span className="rel-person-name">{charName(r.a)}</span>
                  </span>
                  <span className="rel-thread-line">
                    <i />
                    <span className="rel-symbol">❦</span>
                    <i />
                  </span>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                    <span className="rel-avatar" style={{ background: charColor(r.b) }}>
                      {initials(charName(r.b))}
                    </span>
                ...
                <div className="rel-card-body">
                  <div className="rel-desc">
                    {r.description || 'This connection is waiting for its first note.'}
                  </div>
                ...
                <div className="rel-card-footer">
                  <span className="rel-stage-count">
                    {r.stages?.length || 0} {r.stages?.length === 1 ? 'chapter' : 'stages'} mapped
                  </span>
                  <div className="actions-row">
                    <button className="button button-quiet" onClick={() => setEditing({ ...r })}>
                      <Icon icon="fa-solid fa-pen" /> Edit thread
                    </button>
                    <button
                      className="button button-quiet rel-delete"
      ...
      <ConfirmDialog
        ...
        onConfirm={remove}
        title="Unlink these two?"
      >
        The thread between {deleting ? charName(deleting.a) : ''} and{' '}
        {deleting ? charName(deleting.b) : ''} will be removed.
      </ConfirmDialog>
    </div>
  )
}
...
function RelationshipModal({ rel, characters, onChange, onClose, onSave }) {
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title={rel.__new ? 'New relationship' : 'Edit relationship'}
      width={480}
    >
      <div className="field">
        <label>First character</label>
        <Select
          ariaLabel="First character"
          value={rel.a || ''}
          onChange={(value) => set({ a: value })}
      ...
      <div className="field">
        ...
        <Select
          ariaLabel="Second character"
          value={rel.b || ''}
          onChange={(value) => set({ b: value })}
          options={characters.map((c) => ({ value: c.id, label: c.name }))}
        />
      </div>
      <div className="field">
        <label>
          What binds them <span className="hint">(a short line)</span>
        </label>
        <textarea
          spellCheck
          value={rel.description || ''}
          onChange={(e) => set({ description: e.target.value })}
          placeholder="Sister and keeper of secrets…"
        />
      </div>
      <div className="field">
        <label>
          How it grew <span className="hint">(the road between them)</span>
        </label>
        <StageEditor stages={rel.stages || []} onChange={(stages) => set({ stages })} />
      </div>
      <div className="modal-foot">
        ...
        <button className="button button-primary" onClick={onSave}>
          {rel.__new ? 'Weave it' : 'Save changes'}
        </button>
      </div>
    </Modal>
  )
