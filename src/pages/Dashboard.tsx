...
const WRITING_QUOTES = [
  ...
  {
    text: 'The writer who waits for ideal conditions will die without putting a word on paper.',
    author: 'E.B. White',
  },
  ...
  {
    text: 'The very first thing I tell my new students on the first day of a workshop is that good writing is about telling the truth.',
    author: 'Anne Lamott',
  },
  {
...
function NovelCard({
  ...
  const recentlyActive = daysBetween(novel.lastOpened || novel.updatedAt) <= 2
  const isFinished = novel.collection === 'finished'
  const progressPct =
    novel.novelWordGoal > 0
    ...
    openContextMenu(e, [
      ...
      novel.lock
        ? {
            ...
            onClick: () => onToggleLock(novel),
          }
        : {
            label: 'Lock this novel…',
            icon: 'fa-solid fa-lock',
            onClick: () => onToggleLock(novel),
          },
      {
        label: novel.archived ? 'Restore to shelf' : 'Archive',
        icon: 'fa-solid fa-box-archive',
        onClick: () => onArchive(novel),
      },
      ...(!novel.archived
        ? [
            {
              ...
              danger: true,
              onClick: () => onTrash(novel),
            },
          ]
        : []),
      {
        label: 'Duplicate structure as template',
        icon: 'fa-solid fa-copy',
        onClick: () => onDuplicate(novel),
      },
    ])
          ...
          <div className="novel-list-title">
            {novel.pinned && (
              <Icon
                ...
                style={{ marginRight: 5, fontSize: '0.7rem', color: 'var(--moon)' }}
              />
            )}
            {novel.title}
            {isFinished && <span className="finished-badge">✓</span>}
            {novel.sharedRole && (
              <span className="shared-novel-badge">
                <Icon icon="fa-solid fa-user-group" /> Shared
              </span>
            )}
          </div>
        ...
        <div className="novel-list-stats">
          ...
          {progressPct !== null && (
            <span className="novel-list-progress" title={`${progressPct}% of goal`}>
              <span className="novel-progress-bar">
  ...
  return (
    ...
    >
      <div className={`novel-cover cover-${novel.coverStyle || 'moonstone'}`}>
        ...
        {novel.lock && (
          <span className="novel-lock-badge" title="Locked">
            <Icon icon="fa-solid fa-lock" />
          </span>
        )}
        {novel.pinned && (
          <span className="novel-pin-badge" title="Pinned">
            <Icon icon="fa-solid fa-thumbtack" />
          </span>
        )}
        {isFinished && <span className="novel-finished-ribbon">✓</span>}
        {novel.sharedRole && (
          <span className="novel-shared-ribbon" title={`Shared with ${novel.sharedRole} access`}>
            <Icon icon="fa-solid fa-user-group" /> Shared
          </span>
        )}
      </div>
      <div className="novel-menu">
        <button
          ...
          onClick={(e) => {
            e.stopPropagation()
            onPin(novel, !novel.pinned)
          }}
          title={novel.pinned ? 'Unpin' : 'Pin to top'}
        >
          <Icon
            icon="fa-solid fa-thumbtack"
            style={{ color: novel.pinned ? 'var(--moon)' : undefined }}
          />
        </button>
        <button
          className="button button-quiet"
      ...
      <div className="novel-body">
        ...
        <div className={`novel-activity-pulse ${recentlyActive ? 'is-active' : ''}`}>
          <i />
          {recentlyActive ? 'Recently active' : dormant ? 'Quiet for a while' : 'In progress'}
        </div>
        <button
          className="novel-card-continue"
          type="button"
          onClick={(event) => {
        ...
        >
          Continue writing <Icon icon="fa-solid fa-arrow-right" />
        </button>
        {dormant && (
          <div className="novel-dormant-label">
            last opened{' '}
            {dormantDays < 30
              ? `${dormantDays} days ago`
              : dormantDays < 60
                ? 'about a month ago'
                : `${Math.round(dormantDays / 7)} weeks ago`}
          </div>
        )}
        {progressPct !== null && (
          <div
            className="novel-progress-wrap"
            title={`${novelCounts.words || 0} / ${novel.novelWordGoal} words goal`}
          >
  ...
  if (!novels.length)
    return (
      <p style={{ color: 'var(--grey)', margin: 0 }}>
        This is where the first page starts. There's no hurry.
      </p>
    )

  return (
    <div className="rotating-quote">
      <span className="rotating-quote-text">
        "{quote.text}" — {quote.author}
      </span>
      <button
        className="button button-quiet rotating-quote-pin"
        title={pinned ? 'Unpin quote' : 'Pin this quote'}
        onClick={handlePin}
      >
        <Icon
          icon={pinned ? 'fa-solid fa-thumbtack' : 'fa-regular fa-bookmark'}
          style={{ color: pinned ? 'var(--moon)' : undefined }}
        />
      </button>
    </div>
  )
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const params = new URLSearchParams(callbackSearch(window.location, isDesktopRuntime()))
    ...
    ;(async () => {
      try {
        ...
        if (!sharedNovel) {
          throw new Error(
            'The invitation was accepted, but the novel has not arrived yet. Keep the owner online and try the link again.'
          )
        }
        await refreshNovels()
        toast('Shared novel added to your library.')
        if (result.novelId) navigate(`/novel/${result.novelId}`, { replace: true })
      } catch (error) {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'shelf' | 'list'>(() => {
    const saved = localStorage.getItem('moonscribe_view')
    return saved === 'list' || saved === 'shelf' ? saved : 'grid'
  })
  ...
  const [dashboardView, setDashboardView] = useState<
    'home' | 'library' | 'media' | 'journal' | 'insights'
  >(() => {
    const view = new URLSearchParams(window.location.search).get('view')
    return view === 'library' || view === 'media' || view === 'journal' || view === 'insights'
      ? view
      : 'home'
  })
    ...
    ;(async () => {
      try {
        ...
        const recent = [...allChapters]
          ...
          .slice(0, 5)
        const recentNovel = [...activeNovels].sort(
          (a, b) => (b.lastOpened || b.updatedAt || 0) - (a.lastOpened || a.updatedAt || 0)
        )[0]
        const recentContext = readRecentWriting()
        const resumeChapter =
          recentContext?.novelId &&
          allChapters.some(
            (chapter) =>
              chapter.novelId === recentContext.novelId && chapter.id === recentContext.chapterId
          )
            ? allChapters.find(
                (chapter) =>
                  chapter.novelId === recentContext.novelId &&
                  chapter.id === recentContext.chapterId
              )
            : recentNovel
              ? allChapters
                  .filter((chapter) => chapter.novelId === recentNovel.id)
                  .sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0))[0] || null
              : null
        const [today, histories] = await Promise.all([
          Promise.all(activeNovels.map((novel) => todayWords(novel.id))).then((values) =>
        ...
        const reviewData = await Promise.all(
          chaptersByNovel.map(async ({ novel, chapters }) => {
            const [annotations, continuity] = await Promise.all([
              listAnnotations(novel.id),
              continuityReport(novel.id),
            ])
            const unresolvedByChapter = annotations.reduce(
              (counts, annotation) => {
                if (!annotation.resolved && annotation.chapterId)
                  counts[annotation.chapterId] = (counts[annotation.chapterId] || 0) + 1
                return counts
              },
        ...
        const editingQueue = reviewData
          .flatMap(({ novel, chapters, unresolvedByChapter, continuityIssues }) =>
            chapters
              .filter(
                (chapter) =>
                  chapter.status === 'revised' ||
                  unresolvedByChapter[chapter.id] ||
                  continuityIssues.some((issue) => issue.chapterId === chapter.id)
              )
              .map((chapter) => ({
                ...chapter,
                novelTitle: novel.title,
                unresolvedComments: unresolvedByChapter[chapter.id] || 0,
                continuityIssues: continuityIssues.filter((issue) => issue.chapterId === chapter.id)
                  .length,
              }))
          )
          .sort((a, b) => (b.updatedAt || 0) - (a.updatedAt || 0))
        ...
        const health = {
          ...
          unresolvedComments: reviewData.reduce(
            (total, item) =>
              total +
              (Object.values(item.unresolvedByChapter) as number[]).reduce(
                (sum, value) => sum + value,
                0
              ),
            0
          ),
          continuityIssues: reviewData.reduce(
            (total, item) => total + item.continuityIssues.length,
            0
          ),
        }
        if (!cancelled)
          setDashboardData({ today, pulse, recent, resumeChapter, editingQueue, health })
      } catch (error) {
        if (import.meta.env.DEV)
          console.error('Legacy dashboard sidebar data could not load', error)
        if (!cancelled)
          setDashboardData({
            today: 0,
            pulse: [],
  ...
  const backupNow = async () => {
    ...
    downloadBlob(
      ...
      `moonscribe-backup-${new Date().toISOString().slice(0, 10)}.json`
    )
    await setMeta('lastBackupAt', Date.now())
    setShowNudge(false)
    toast('Backup downloaded — kept safe.')
    emitSound('action.success')
  }
  ...
  const setView = (v) => {
    setViewMode(v)
    localStorage.setItem('moonscribe_view', v)
  }
  ...
  const toggleNovelLock = async (novel) => {
    if (novel.lock) {
      ...
      if (pass === null) return
      if (!(await verifyLock(novel.lock, pass))) {
        toast("That didn't match — lock unchanged.")
        return
      }
      ...
      const pass = window.prompt(
        `Choose a passphrase to lock "${novel.title || 'this novel'}".\nNo recovery — keep it safe.`
      )
      if (pass === null) return
      if (!pass.trim()) {
        toast('A passphrase is needed.')
        return
      }
      const again = window.prompt('Enter it again to confirm.')
      if (again === null) return
      if (pass !== again) {
        toast("The two entries didn't match.")
        return
      }
  ...
  const handleCreate = async ({ title, blurb, coverStyle, genres, series, template }) => {
    if (novels.filter((novel) => !novel.archived).length >= MAX_NOVELS_PER_USER) {
      toast(
        `MoonScribe supports up to ${MAX_NOVELS_PER_USER} active novels per user. Archive one before creating another.`
      )
      return false
    }
  ...
  const handleTrash = async () => {
    if (!trashNovelTarget) return
    await trashNovel(trashNovelTarget.id)
    setTrashNovelTarget(null)
    await refreshNovels()
    toast('Moved to the Trash — recoverable for 30 days.', {
      label: 'Undo',
      run: async () => {
        const { restoreTrashed } = await import('../db/trash')
        await restoreTrashed('novels', trashNovelTarget.id)
        await refreshNovels()
      },
    })
    emitSound('action.destructive')
  ...
  const active = novels.filter(
    (n) => !n.archived && (!n.sharedExpiresAt || n.sharedExpiresAt > Date.now())
  )
  ...
  const resumeNovel = active.sort(
    (a, b) => (b.lastOpened || b.updatedAt || 0) - (a.lastOpened || a.updatedAt || 0)
  )[0]
  const attentionCount =
    (dashboardData.health?.unresolvedComments || 0) + (dashboardData.health?.continuityIssues || 0)
  const resumeChapter = dashboardData.resumeChapter
  const openResume = () => {
    if (!resumeNovel) return
    navigate(`/novel/${resumeNovel.id}`, {
      state: resumeChapter ? { chapterId: resumeChapter.id } : undefined,
    })
  }
  ...
  return (
    <div className="app dashboard-app">
      <DashboardSidebar
        ...
        openAccountCentre={openAccountCentre}
      />
        ...
        <div className={`dashboard dashboard-layout-${settings.appLayout || 'studio'}`}>
          <div className="topbar">
            <Sheet open={mobileNavOpen} onOpenChange={setMobileNavOpen}>
              ...
              <SheetContent
                side="left"
                className="dashboard-mobile-sheet animate-in slide-in-from-left duration-300 ease-out"
              >
                ...
                <nav className="dashboard-mobile-nav" aria-label="Dashboard navigation">
                  {[
                    ...
                    ['media', 'Media', 'fa-solid fa-images'],
                    ['journal', 'Journal', 'fa-solid fa-book'],
                    ['insights', 'Insights', 'fa-solid fa-chart-line'],
                  ].map(([key, label, icon]) => (
                    <Button
                      key={key}
                      variant={dashboardView === key ? 'secondary' : 'ghost'}
                      className="justify-start"
                      onClick={() => {
                        setDashboardView(key as typeof dashboardView)
                  ...
                  {hasRole('admin') && (
                    <Button
                      variant="ghost"
                      className="dashboard-mobile-admin-link justify-start"
                      onClick={() => {
                        navigate('/admin')
                        setMobileNavOpen(false)
                      }}
            ...
            <Button variant="outline" className="dashboard-command-launcher" onClick={openSearch}>
              <Icon icon="fa-solid fa-magnifying-glass" /> Search MoonScribe <kbd>Ctrl K</kbd>
            </Button>
          ...
          <div className="dashboard-workspace">
            ...
            {dashboardView === 'library' && (
              <section className="dashboard-library" ref={libraryRef} aria-busy={libraryLoading}>
                ...
                {novels.length === 0 ? (
                  <section className="dashboard-empty-view">
                    ...
                    <p>
                      Your novel library is empty right now. Create the first one and MoonScribe
                  ...
                  <>
                    <div className="dashboard-tools dashboard-library-tools">
                      <div className="dashboard-library-heading">
                        <div>
                          <h2>Your Library</h2>
                          <p>Find a story, continue one, or begin something new.</p>
                        </div>
                        <div className="dashboard-library-heading-actions">
                          <button className="button button-primary" onClick={() => setNewOpen(true)}>
                            <Icon icon="fa-solid fa-plus" style={{ marginRight: 6 }} /> New novel
                          </button>
                          <button className="button button-ghost" type="button" onClick={() => navigate('/trash')}>
                            <Icon icon="fa-solid fa-trash-can" style={{ marginRight: 6 }} /> Trash
                          </button>
                        </div>
                      </div>
                        ...
                        <div style={{ position: 'relative', flex: 1, maxWidth: 480 }}>
                          ...
                          {crossResults && q.length >= 3 && (
                            <div className="cross-search-results">
                              {crossResults.chapters.length > 0 && (
                                <div className="cross-section">
                                  ...
                                  {crossResults.chapters.slice(0, 4).map((r) => (
                                    <button
                                      key={r.id}
                                      className="cross-result"
                                      onClick={() =>
                                        navigate(`/novel/${r.novelId}`, {
                                ...
                                <div className="cross-section">
                                  ...
                                  {crossResults.characters.slice(0, 4).map((r) => (
                                    <button
                                      key={r.id}
                                      className="cross-result"
                                      onClick={() => navigate(`/novel/${r.novelId}/characters`)}
                                    >
                                ...
                                <div className="cross-section">
                                  ...
                                  {crossResults.notes.slice(0, 3).map((r) => (
                                    <button
                                      key={r.id}
                                      className="cross-result"
                                      onClick={() => navigate(`/novel/${r.novelId}/notes`)}
                                    >
                              ...
                              {!crossResults.chapters.length &&
                                !crossResults.characters.length &&
                                !crossResults.notes.length && (
                                  <div
                                    style={{
                                      padding: '12px 14px',
                                      color: 'var(--grey)',
                      ...
                      <div className="dashboard-tools-bottom">
                        <div className="dashboard-sort">
                          ...
                          <Select
                            ariaLabel="Genre"
                            value={genreFilter || ''}
                            onChange={(value) => setGenreFilter(value || null)}
                            options={[
                              { value: '', label: 'All genres' },
                              ...((Array.isArray(allGenres) && allGenres.length ? allGenres : GENRES) as string[]).map((genre) => ({ value: genre, label: genre })),
                            ]}
                          />
                          ...
                          <Select
                            ...
                            onChange={(value) => setCollectionFilter(value || null)}
                            options={[
                              { value: '', label: 'All statuses' },
                              ...COLLECTIONS.filter((collection) => collection.key).map((collection) => ({ value: collection.key || '', label: collection.label })),
                            ]}
                          />
                          ...
                          <div className="view-toggle">
                            ...
                            <button
                              className={`view-btn ${viewMode === 'shelf' ? 'active' : ''}`}
                            ...
                            >
                              ...
                              <span className="view-btn-label">Shelf</span>
                            </button>
                            <button
                    ...
                    <div className="library-result-bar">
                      <span>
                        <strong>{visible.length}</strong> of {active.length}{' '}
                        {active.length === 1 ? 'story' : 'stories'}
                      </span>
                      {(q || genreFilter || collectionFilter) && (
                ...
                {!libraryLoading && !visible.length && (
                  <LibraryEmptyState
                    ...
                    hasFilters={Boolean(q || genreFilter || collectionFilter)}
                    onCreate={() => setNewOpen(true)}
                    onClear={() => {
                      setQuery('')
                      setGenreFilter(null)
                    ...
                    <div className="series-section" key={g.name || '__none'}>
                      {g.name && (
                        <div className="section-heading series-heading">
                          <Icon icon="fa-solid fa-layer-group" style={{ marginRight: 8 }} />{' '}
                          {g.name}
                          <span className="hint">
                            {g.items.length} {g.items.length === 1 ? 'novel' : 'novels'}
                          </span>
                        </div>
                      )}
                      {viewMode === 'list' ? (
                        <div className="novel-list">
                          ...
                          {g.items.length === 0 && novels.length > 0 && (
                            <div className="empty">
                              ...
                              <p>
                                {q
                                  ? `No novel matches "${query}".`
                                  : `No ${genreFilter} novels yet.`}{' '}
                                Try another search.
                              </p>
                              <button
                        ...
                        <div className="novel-bookshelf-wrap">
                          <BookShelf
                            books={g.items.map((n) => ({
                              ...
                              novelId: n.id,
                              author: syncUsername,
                              cover: n.layout?.cover?.frontImage || n.cover,
                              coverDesign: n.layout?.cover,
                            }))}
                            counts={counts}
                            initialView="shelf"
                          ...
                          {g.items.length === 0 && novels.length > 0 && (
                            <div className="empty" style={{ gridColumn: '1 / -1' }}>
                              ...
                              <p>
                                {q
                                  ? `No novel matches "${query}".`
                                  : `No ${genreFilter || collectionFilter} novels yet.`}{' '}
                                Try another search.
                              </p>
                              <button
                                className="button button-ghost"
                        ...
                        <div className="novel-grid" aria-label="Novel grid">
                          {g.items.map((n) => (
                            <NovelCard key={n.id} novel={n} {...sharedCardProps} listView={false} />
                          ))}
                        </div>
                      )}
                    </div>
            ...
            {dashboardView === 'media' && (
              <GlobalMedia
                novels={novels}
                onOpenNovel={(novelId) => navigate(`/novel/${novelId}/media`)}
              />
            )}
            {dashboardView === 'journal' && (
              <>
                <MobileDashboardHeader
                  username={syncUsername}
                  syncStatus={syncStatus}
                ...
                <DashboardJournal
                  novel={resumeNovel}
                  analytics={analytics}
                  onOpen={() => resumeNovel && navigate(`/novel/${resumeNovel.id}/writing-journal`)}
                />
              </>
            )}
            {dashboardView === 'insights' && (
              <DashboardInsights
                analytics={analytics}
                data={dashboardData}
                onOpen={() => resumeNovel && navigate(`/novel/${resumeNovel.id}/analytics`)}
              />
            )}
          </div>
      ...
      <ConfirmDialog
        open={!!trashNovelTarget}
        onClose={() => setTrashNovelTarget(null)}
        onConfirm={handleTrash}
        title="Move this novel to the Trash?"
      >
        “{trashNovelTarget?.title}” and its chapters, characters, notes, and designs will be kept
        safely for 30 days.
      </ConfirmDialog>
      <ConfirmDialog
      ...
      >
        "{deleteNovelTarget?.title}" and all its chapters, characters and notes will be removed from
...
function MobileDashboardNav({
  ...
  onNavigate: (view: MobileDashboardView) => void
  onNew: () => void
  onSettings: () => void
}) {
  const items = [
  ...
  return (
    <nav
      className="mobile-dashboard-nav flex items-end justify-around gap-1 border-t border-white/10 bg-[#0b0b0f]/95 px-2 py-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] backdrop-blur-xl"
    ...
    >
      {items.slice(0, 2).map(([key, label, icon]) => (
        <button
          ...
          className={`grid min-h-11 min-w-11 place-items-center gap-1 rounded-lg px-2 py-1 text-[.62rem] transition-colors focus-visible:outline-2 focus-visible:outline-[#c79b53] ${view === key ? 'active text-[#c79b53]' : 'text-[#858188] hover:text-[#c79b53]'}`}
      ...
      <button
        className="mobile-dashboard-create grid h-12 w-12 min-w-12 place-items-center rounded-full border-2 border-[#c79b53] bg-[#141218] text-[#c79b53] shadow-[0_5px_20px_rgba(0,0,0,.45)] focus-visible:outline-2 focus-visible:outline-[#f1d28a]"
        onClick={onNew}
        aria-label="Create new"
      >
        <Icon icon="fa-solid fa-plus" />
      </button>
      {items.slice(2, 3).map(([key, label, icon]) => (
        <button
          key={key}
          aria-current={view === key ? 'page' : undefined}
          className={`grid min-h-11 min-w-11 place-items-center gap-1 rounded-lg px-2 py-1 text-[.62rem] transition-colors focus-visible:outline-2 focus-visible:outline-[#c79b53] ${view === key ? 'active text-[#c79b53]' : 'text-[#858188] hover:text-[#c79b53]'}`}
...
function DashboardSkeleton() {
  return (
    <div className="dashboard-skeleton" aria-label="Loading your library" aria-live="polite">
      {[0, 1, 2].map((item) => (
        <div className="skeleton-novel" key={item}>
...
function HeroCard({
  ...
  return (
    <div
      className={`hero-card ${coverClass}`}
      onClick={() => onOpen(novel)}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => e.key === 'Enter' && onOpen(novel)}
      ...
      <div className="hero-body">
        ...
        {chapter && (
          <div className="hero-chapter">
            {chapter.title || 'Untitled chapter'} · last edited {timeAgo(chapter.updatedAt)}
          </div>
        )}
        {chapter?.meta?.location && (
          <div className="hero-context">
            Scene: {chapter.meta.location}
            {chapter.meta.tone ? ` · Tone: ${chapter.meta.tone}` : ''}
          </div>
        )}
        {lastLine(chapter?.content) && (
          <div className="hero-last-line">“{lastLine(chapter.content)}”</div>
        )}
        <div className="hero-meta">
          {formatWords(todayWords)} / {formatWords(novel.goalWords || 500)} today ·{' '}
          {counts?.chapters || 0} chapters · {streak || 0}-day streak
        </div>
        <div className="hero-actions">
          ...
          <button
            className="button button-quiet hero-secondary-action"
            onClick={(event) => {
              event.stopPropagation()
              onOpenChapter()
            }}
...
function DashboardSidebar({
  ...
  const item = (key, label, icon, onClick) => (
    <button
      className={`dashboard-sidebar-item ${view === key ? 'active' : ''}`}
      onClick={onClick}
  ...
  return (
    <aside className={`dashboard-sidebar ${collapsed ? 'collapsed' : ''}`}>
      <div className="dashboard-sidebar-brand">
        ...
        <span>
          MoonScribe<small>Stories, quietly written.</small>
        </span>
        <button
          onClick={onToggle}
          aria-label={collapsed ? 'Expand dashboard sidebar' : 'Collapse dashboard sidebar'}
        >
          <Icon icon={collapsed ? 'fa-solid fa-angles-right' : 'fa-solid fa-angles-left'} />
        </button>
      </div>
      <nav className="dashboard-sidebar-nav" aria-label="Dashboard navigation">
        ...
        <button
          ...
          onClick={onSearch}
          title={collapsed ? 'Search MoonScribe' : undefined}
        >
          <Icon icon="fa-solid fa-magnifying-glass" />
          <span className="dashboard-sidebar-label">Search</span>
        </button>
      </nav>
      {showCurrentStory && (
        <div className="dashboard-current-story">
          ...
          <button onClick={onContinue} title={collapsed ? 'Continue writing' : undefined}>
            {currentStoryCover ? (
              <img className="dashboard-current-story-cover" src={currentStoryCover} alt="" />
            ) : (
              <Icon icon="fa-solid fa-sparkles" />
            )}
            <span className="dashboard-sidebar-label">
              {recent[0]?.novelTitle || 'Choose a story'}
              <small>{resumeChapter?.title || 'Start where you left off'}</small>
            </span>
          </button>
        </div>
      )}
      <ProfilePopover
        ...
        profile={{
          username: syncUsername || 'local-writer',
          avatarUrl: syncAvatar,
          presence:
            syncStatus === 'synced' ? 'online' : syncStatus === 'syncing' ? 'idle' : 'offline',
          showActivity: false,
        }}
        actions={[
          {
            ...
            ? [
                {
                  ...
                  onClick: async () => {
                    await navigator.clipboard?.writeText(syncUsername)
                  },
                },
              ]
            : []),
          {
            label: syncUsername ? 'Sign out' : 'Exit offline mode',
      ...
      <div className="dashboard-sidebar-bottom">
        ...
        >
          <Icon icon="fa-solid fa-plus" />
          <span className="dashboard-sidebar-label">New story</span>
        </button>
        <button
          className="dashboard-sidebar-item"
          onClick={onSettings}
        ...
        {onAdmin && (
          <button
            className="dashboard-sidebar-item dashboard-admin-tab"
            onClick={onAdmin}
        ...
        <div
          className={`dashboard-account-card status-${syncStatus || 'offline'}`}
          title={
            collapsed ? `${syncUsername || 'Local writer'} · ${syncStatus || 'offline'}` : undefined
          }
          onContextMenu={openProfileMenu}
        >
          <button
            className="dashboard-account-identity"
            type="button"
          ...
          >
            <span className="dashboard-account-avatar">
              ...
              <i
                className={`dashboard-account-presence ${syncStatus === 'synced' ? 'online' : syncStatus === 'error' ? 'error' : ''}`}
              />
            </span>
            <span className="dashboard-sidebar-label dashboard-account-copy">
              ...
              <small>
                {syncStatus === 'synced'
                  ? 'Synced'
                  : syncStatus === 'local'
                    ? 'Saved locally · cloud sync queued'
                    : syncStatus === 'syncing'
                      ? 'Syncing…'
          ...
          <div className="dashboard-account-actions dashboard-sidebar-label">
            <button type="button" onClick={onSync} aria-label="Sync now" title="Sync now">
              <Icon icon="fa-solid fa-rotate" />
            </button>
            {syncProvider === 'discord' && (
              <span title="Connected with Discord" aria-label="Connected with Discord">
                <Icon icon="fa-brands fa-discord" />
              </span>
            )}
            <button type="button" onClick={onSettings} aria-label="Open settings" title="Settings">
              <Icon icon="fa-solid fa-gear" />
            </button>
          </div>
        </div>
        {!collapsed && (
          <div className="moonscribe-legal-footer">
            ...
            <a
              href="https://abr.business.gov.au/ABN/View?abn=47600241842"
              target="_blank"
              rel="noreferrer"
            >
              ABN 47 600 241 842
...
function RecentTouched({ chapters, onOpenChapter }) {
  ...
  return (
    <section className="dashboard-recent-touched">
      ...
      <div>
        {chapters.slice(0, 3).map((chapter) => (
          <button key={chapter.id} onClick={() => onOpenChapter(chapter)}>
            ...
            <small>{timeAgo(chapter.updatedAt)}</small>
          </button>
        ))}
      </div>
    </section>
...
function LibraryEmptyState({ hasNovels, hasArchived, hasFilters, onCreate, onClear }) {
  const title = hasFilters
    ? 'No stories match that view.'
    : hasArchived
      ? 'Your active shelf is quiet.'
      : hasNovels
        ? 'No active stories yet.'
        : 'Stories will appear here.'
  const detail = hasFilters
    ? 'Try clearing a filter or searching for another title.'
    : hasArchived
      ? 'Your archived stories are safe. Restore one when you are ready to return.'
      : 'Create a story and make this library yours.'
  return (
    <section className="library-empty-state" aria-live="polite">
      <div className="library-empty-orbit" aria-hidden="true">
        <span>✦</span>
        <i />
        <b />
      </div>
      <span className="dashboard-section-label">Your library</span>
      <h2>{title}</h2>
      <p>{detail}</p>
      <div className="actions-row">
        {hasFilters ? (
...
function DashboardJournal({ novel, analytics, onOpen }) {
  ...
  const monthlyWords = analytics.monthlyWords || 0

  return (
    <section className="dashboard-empty-view dashboard-journal-home relative isolate !min-h-full !items-stretch !justify-items-stretch !gap-0 !p-0 overflow-hidden rounded-[28px] border border-white/[0.09] bg-[#0c0d11] text-[#f5f0e8] shadow-[0_24px_80px_rgba(0,0,0,0.24)]">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -right-24 -top-32 h-96 w-96 rounded-full bg-[radial-gradient(circle,rgba(190,132,42,0.2),transparent_68%)] blur-2xl"
      />
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -bottom-48 left-1/4 h-96 w-96 rounded-full bg-[radial-gradient(circle,rgba(86,102,148,0.16),transparent_68%)] blur-3xl"
      />

      <div className="relative z-10 grid min-h-full grid-cols-1 content-start gap-4 p-4 sm:p-6 lg:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)] lg:gap-5 lg:p-7">
        <div className="relative flex min-h-[330px] flex-col justify-between overflow-hidden rounded-[22px] border border-white/[0.1] bg-white/[0.045] p-6 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-xl sm:min-h-[410px] sm:p-8">
          <div
            aria-hidden="true"
            className="pointer-events-none absolute inset-0 bg-[linear-gradient(135deg,rgba(255,255,255,0.06),transparent_38%,rgba(190,132,42,0.05))]"
          />
          <div className="relative">
            <div className="flex items-center justify-between gap-4">
              <span className="dashboard-section-label !text-[0.68rem] !tracking-[0.18em] !text-[#c58a2c]">
                Writing journal
              </span>
              <span
                className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-[#c58a2c]/25 bg-[#c58a2c]/10 text-[#d49a38]"
                aria-hidden="true"
              >
                <Icon icon="fa-solid fa-feather-pointed" />
              </span>
            </div>
            <h1 className="mt-7 max-w-[10ch] !text-[clamp(2.45rem,5vw,4.5rem)] !leading-[0.94] !tracking-[-0.055em]">
              Your writing, remembered.
            </h1>
            <p className="mt-5 max-w-[35ch] !text-base !leading-7 !text-white/60">
              {streak
                ? `${streak} days in your current rhythm.`
                : 'Start a writing session and MoonScribe will keep the rhythm here.'}
            </p>
          </div>
          <div className="relative mt-12 flex items-end justify-between gap-5 border-t border-white/[0.1] pt-5">
            <div>
              <span className="block text-[0.62rem] font-semibold uppercase tracking-[0.18em] text-white/35">
                A place for the in-between
              </span>
              <span className="mt-2 block max-w-[28ch] font-[var(--font-heading)] text-lg leading-6 text-white/80">
                Keep the fragments close until they find their chapter.
              </span>
            </div>
            <span
              className="hidden text-5xl font-light tracking-[-0.08em] text-white/[0.08] sm:block"
              aria-hidden="true"
            >
              01
            </span>
        ...
        <div className="grid content-start gap-4 sm:grid-cols-2 lg:grid-cols-1">
          <div className="grid grid-cols-2 gap-4 sm:col-span-2 lg:col-span-1">
            <div className="rounded-[22px] border border-white/[0.1] bg-white/[0.045] p-5 backdrop-blur-xl">
              <span className="block text-[0.62rem] font-semibold uppercase tracking-[0.16em] text-white/40">
                Current rhythm
              </span>
              <strong className="mt-5 block text-4xl font-medium tracking-[-0.06em] text-[#d49a38]">
                {streak}
              </strong>
              <span className="mt-1 block text-sm text-white/55">days in motion</span>
            </div>
            <div className="rounded-[22px] border border-white/[0.1] bg-white/[0.045] p-5 backdrop-blur-xl">
              <span className="block text-[0.62rem] font-semibold uppercase tracking-[0.16em] text-white/40">
                This month
              </span>
              <strong className="mt-5 block text-4xl font-medium tracking-[-0.06em] text-[#f5f0e8]">
                {formatWords(monthlyWords)}
              </strong>
              <span className="mt-1 block text-sm text-white/55">words written</span>
            </div>
          </div>

          <div className="flex min-h-[190px] flex-col justify-between rounded-[22px] border border-[#c58a2c]/20 bg-[linear-gradient(145deg,rgba(197,138,44,0.13),rgba(255,255,255,0.035)_56%)] p-6 backdrop-blur-xl sm:col-span-2 lg:col-span-1">
            <div className="flex items-start justify-between gap-4">
              <span
                className="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-[#c58a2c]/25 bg-[#c58a2c]/10 text-[#d49a38]"
                aria-hidden="true"
              >
                <Icon icon="fa-solid fa-sparkles" />
              </span>
              <span className="text-[0.62rem] font-semibold uppercase tracking-[0.16em] text-[#d49a38]/70">
                A quiet prompt
              </span>
            </div>
            <div className="mt-7">
              <strong className="block font-[var(--font-heading)] text-xl leading-6 text-white/90">
                A quiet place for the pages between pages.
              </strong>
              <span className="mt-2 block text-sm leading-6 text-white/55">
                Keep a thought, a turning point, or a note for your future self.
              </span>
            </div>
          </div>
        </div>
        ...
        {novel && (
          <button
            className="button button-primary dashboard-journal-cta !m-0 !w-fit !max-w-full !justify-self-start"
            onClick={onOpen}
          >
            <span>Open {novel.title}'s journal</span>
...
function DashboardInsights({ analytics, data, onOpen }) {
  return (
    <section className="dashboard-empty-view">
      ...
      <p>
        {formatWords(analytics.monthlyWords)} words this month · {data.health?.chapters || 0}{' '}
        chapters in your library.
      </p>
...
function DashboardWidgets({
  ...
  const widgetContent = {
    today: (
      <>
        <div className="dashboard-widget-heading">
          <Icon icon="fa-solid fa-feather-pointed" /> Today
        </div>
        <strong className="dashboard-widget-number">{formatWords(data.today)}</strong>
        <span>words written</span>
        <div className="dashboard-widget-summary">
          {analytics.streak ? `${analytics.streak}-day writing streak` : 'A fresh page is waiting.'}
        </div>
      </>
    ),
    pulse: (
      <>
        <div className="dashboard-widget-heading">
          <Icon icon="fa-solid fa-wave-square" /> Writing pulse
        </div>
        <div className="writing-pulse" aria-label="Words written over the last seven days">
          {data.pulse.map((day) => (
            <span
              key={day.date}
              className="writing-pulse-bar"
              style={{ height: `${Math.max(8, (day.words / maxPulse) * 100)}%` }}
              title={`${day.words} words`}
            />
          ))}
        </div>
        <span>
          Last 7 days · {formatWords(data.pulse.reduce((total, day) => total + day.words, 0))} words
        </span>
      </>
    ),
    recent: (
      <>
        <div className="dashboard-widget-heading">
          <Icon icon="fa-regular fa-clock" /> Recent chapters
        </div>
        <div className="dashboard-recent-list">
          {data.recent.length ? (
            data.recent.slice(0, 4).map((chapter) => (
              <button key={chapter.id} onClick={() => onOpenChapter(chapter)}>
                <span>{chapter.title || 'Untitled chapter'}</span>
                <small>
                  {chapter.novelTitle} · {timeAgo(chapter.updatedAt)}
                </small>
              </button>
            ))
          ) : (
    ...
    editing: (
      <>
        <div className="dashboard-widget-heading">
          <Icon icon="fa-solid fa-pen-ruler" /> Editing queue
        </div>
        <div className="dashboard-recent-list">
          {data.editingQueue.length ? (
            data.editingQueue.slice(0, 4).map((chapter) => (
              <button key={chapter.id} onClick={() => onOpenChapter(chapter)}>
                <span>{chapter.title || 'Untitled chapter'}</span>
                <small>
                  {chapter.unresolvedComments
                    ? `${chapter.unresolvedComments} unresolved comment${chapter.unresolvedComments === 1 ? '' : 's'}`
                    : chapter.continuityIssues
                      ? `${chapter.continuityIssues} continuity item${chapter.continuityIssues === 1 ? '' : 's'}`
                      : 'Ready for revision'}
                </small>
              </button>
            ))
            ...
            <span className="muted">
              Revision-ready chapters and open comments will appear here.
            </span>
          )}
        </div>
      </>
    ...
    health: (
      <>
        <div className="dashboard-widget-heading">
          <Icon icon="fa-solid fa-heart-pulse" /> Novel health
        </div>
        <strong className="dashboard-widget-number">{data.health?.chapters || 0}</strong>
        <span>
          chapters · {data.health?.drafted || 0} drafted · {data.health?.editing || 0} editing ·{' '}
          {data.health?.complete || 0} complete
        </span>
        <div className="dashboard-widget-summary">
          {data.health?.unresolvedComments || 0} unresolved comments ·{' '}
          {data.health?.continuityIssues || 0} continuity items
        </div>
      </>
    ),
    quickCapture: (
      <>
        <div className="dashboard-widget-heading">
          <Icon icon="fa-solid fa-bolt" /> Quick capture
        </div>
        <textarea
          className="dashboard-quick-capture"
          value={quickCapture}
          onChange={(event) => onQuickCaptureChange(event.target.value)}
          placeholder="A line, scene idea, or detail before it disappears…"
          aria-label="Quick capture note"
        />
        <button
          className="button button-ghost dashboard-quick-capture-save"
          onClick={onSaveQuickCapture}
          disabled={!quickCapture.trim()}
        >
          Save to current story
        </button>
      </>
    ),
    progress: (
      <>
        <div className="dashboard-widget-heading">
          <Icon icon="fa-solid fa-book-open" /> Story progress
        </div>
        <strong className="dashboard-widget-number">
          {formatWords(
            (Object.values(counts) as { words?: number; chapters?: number }[]).reduce(
              (total, count) => total + (count.words || 0),
              0
            )
          )}
        </strong>
        <span>
          words across{' '}
          {(Object.values(counts) as { words?: number; chapters?: number }[]).reduce(
            (total, count) => total + (count.chapters || 0),
            0
          )}{' '}
          chapters
        </span>
        <div className="dashboard-widget-summary">
          {analytics.inProgress} active {analytics.inProgress === 1 ? 'story' : 'stories'} ·{' '}
          {formatWords(analytics.monthlyWords)} words this month
        </div>
      </>
    ),
  }
  ...
  return (
    <section
      className={`dashboard-widget-area ${customizing ? 'is-customizing' : ''}`}
      aria-label="Writing dashboard"
    >
      {customizing && (
        <div className="dashboard-widget-picker">
          {DEFAULT_DASHBOARD_WIDGETS.map((widget) => (
            <button
              key={widget}
              className={widgets.includes(widget) ? 'active' : ''}
      ...
      <div className="dashboard-widget-grid">
        {widgets.map((widget) => (
          ...
          >
            {customizing && (
              <span className="dashboard-drag-handle" aria-hidden="true">
    ...
    <div className="genre-chips">
      {GENRES.map((g) => (
        <button
          key={g}
          type="button"
          className={`chip ${value.includes(g) ? 'active' : ''}`}
          onClick={() => toggle(g)}
        >
          {g}
        </button>
...
function NewNovelModal({ open, onClose, onCreate }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (result === false) setCreating(false)
    } catch (cause) {
      setError(cause?.message || 'We couldn’t create that novel. Please try again.')
      setCreating(false)
    }
  }
  ...
  return (
    <Modal
      ...
      width={1100}
      className="new-novel-modal"
    >
      <div className="novel-create-layout">
        <aside className="novel-create-atmosphere" aria-hidden="true">
          <div className="novel-create-quote">
            Every
            <br />
            great story
            <br />
            ...
            <br />
            somewhere.
            <span className="novel-create-divider" />{' '}
            <small>
              A NEW CHAPTER
              <br />
              AWAITS.
        ...
        <form className="novel-create-form" onSubmit={submit}>
          <header className="novel-create-header">
            <div className="novel-create-kicker">
              <span className="novel-create-book">
                <Icon icon="fa-solid fa-book-open" />
              </span>{' '}
              Create a new novel
            </div>
            <h2>Begin a new novel</h2>
            <p>Give your story a home, and take the first step.</p>
          </header>
          <div className="novel-create-fields">
            <div className="field">
              ...
              <input
                ...
                placeholder="A title, or none yet…"
                autoFocus
              />
            </div>
            <div className="field novel-blurb-field">
              <label htmlFor="novel-blurb">
                One-line idea <span className="hint">(optional)</span>
              </label>
              <textarea
                id="novel-blurb"
                value={blurb}
                ...
                placeholder="Two strangers, one small town…"
                rows={2}
              />
              <span className="novel-character-count">{blurb.length}/200</span>
            </div>
            <div className="field">
              <label>Starting shape</label>
              <div className="template-grid" role="radiogroup" aria-label="Starting shape">
                {NOVEL_TEMPLATES.map((t) => {
                  ...
                  return (
                    <button
                      ...
                      className={`template-card ${template === t.key ? 'selected' : ''}`}
                    ...
                    >
                      ...
                      <span className="template-indicator">{template === t.key ? '✓' : ''}</span>
                    </button>
                  )
                })}
              </div>
            ...
            <div className="field">
              <label htmlFor="novel-series">
                Series <span className="hint">(optional)</span>
              </label>
              <input
                id="novel-series"
                value={series}
                onChange={(e) => setSeries(e.target.value)}
                placeholder="e.g. The Tide Trilogy"
              />
            </div>
            <div className="field">
              <label>
          ...
          {error && (
            <p className="novel-create-error" role="alert">
              {error}
            </p>
          )}
          <footer className="modal-foot">
            ...
            <button type="submit" className="button button-primary" disabled={creating}>
              <Icon
                icon={creating ? 'fa-solid fa-spinner fa-spin' : 'fa-solid fa-feather-pointed'}
              />{' '}
              {creating ? 'Creating your novel…' : 'Begin writing'}{' '}
              <Icon icon="fa-solid fa-arrow-right" />
            </button>
          </footer>
        </form>
  ...
  return (
    <Modal open={!!novel} onClose={onClose} title="About this novel" width={520}>
      ...
      <div className="field">
        <label>
          Word count goal{' '}
          <span className="hint">(optional — shows a progress bar on the card)</span>
        </label>
        <input
          type="number"
          min="0"
...
function CoverModal({ novel, onClose, onDone }) {
  ...
  return (
    <Modal open={!!novel} onClose={onClose} title="Cover" width={440}>
        ...
        <div className="swatch-row">
          {COVER_STYLES.map((c) => (
            <button
              ...
              onClick={() => {
                setCoverStyle(c.key)
                updateNovel(novel.id, { coverStyle: c.key })
                onDone()
              }}
            />
          ))}
      ...
      <div className="field">
        <label>
          Picture <span className="hint">(optional — stays on this device)</span>
        </label>
        <input type="file" accept="image/*" onChange={pickImage} />
      </div>
      {novel?.cover && (
        <div className="actions-row" style={{ marginBottom: 12 }}>
          <button className="button button-ghost" onClick={removeImage}>
            Remove picture
          </button>
        </div>
      )}
