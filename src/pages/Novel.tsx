...
import { deleteYjsThread, toggleYjsThreadResolved, upsertYjsThread } from '../sync/commentThreads'
...
const InteriorLayoutPage = lazy(() => import('./InteriorLayoutPage'))
...
import { listMoodboardSidebar, deleteTile } from '../db/moodboard'
import { markPerformance } from '../utils/performance'
import { emitSound } from '../utils/sounds'
import {
  openEditorCollaboration,
  collaborativeWriteEnabled,
} from '../sync/editorCollaborationAdapter'
...
const SceneField = memo(function SceneField({
  label,
  value,
  onCommit,
  options,
...
export default function Novel() {
  ...
  useEffect(() => {
    ...
    navigate(location.pathname, {
      replace: true,
      state: { ...location.state, openChapters: false },
    })
  ...
  const refreshMediaFiles = useCallback(async () => {
    setMediaFiles((await listMoodboardSidebar(id)).filter((item) => item.kind === 'image'))
  }, [id])
  ...
  const sessionAutoPausedRef = useRef(false)
  const sessionLastActivityRef = useRef(Date.now())
  ...
  const liveDocRef = useRef<any>(null)
  ...
  const ownerAway = collaborationRoomState
    ? collaborationRoomState === 'owner-away'
    : Boolean(novel?.sharedReadOnly)
  const canEditSharedNovel =
    !novel?.sharedRole || (collaborativeWriteEnabled && novel.sharedRole === 'editor' && !ownerAway)
  const canProofreadSharedNovel =
    !novel?.sharedRole ||
  ...
  useEffect(() => {
    const onCollaboration = (event) => {
      if (String(event.detail?.novelId || '') !== String(id)) return
      if (event.detail?.room?.state) setCollaborationRoomState(event.detail.room.state)
      else if (event.detail?.status === 'connected') setCollaborationRoomState('live')
    }
    window.addEventListener('moonscribe:collaboration', onCollaboration)
    return () => window.removeEventListener('moonscribe:collaboration', onCollaboration)
  }, [id])
  ...
  // Collaboration transport is an adapter around the canonical Editor. The
  // feature flag intentionally defaults to read-only for shared writers until
  // concurrent browser editing has passed the release smoke matrix.
  useEffect(() => {
    let cancelled = false
    let session: any = null
    const chId = chapter?.id
    if (!novel?.sharedRole || !chId) {
    ...
    openEditorCollaboration({
      ...
      controller: liveEditorRef.current,
      onRemoteTitle: (title) => setTitleDraft(title),
      onStatus: (status) =>
        window.dispatchEvent(
          new CustomEvent('moonscribe:collaboration', { detail: { novelId: id, status } })
        ),
    })
      .then((next) => {
        if (cancelled) return next.close()
        session = next
        liveDocRef.current = next.doc
        liveSessionRef.current = next
        return undefined
      })
      .catch(() => {
        if (!cancelled) setCollaborationRoomState('offline')
      })
    return () => {
      cancelled = true
      liveDocRef.current = null
  ...
  }, [id, chapter?.id, chapter?.content, chapter?.title, novel?.sharedRole, canEditSharedNovel])
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const first =
        chs.find((c) => recent?.novelId === id && c.id === recent.chapterId) ||
        chs.find((c) => c.id === n.lastChapterId) ||
        chs[0]
      if (first) {
      ...
      const kindMap = { place: 'place', faction: 'faction', item: 'artefact', creature: 'creature' }
      ...
      if (first) {
        if (
          recent?.novelId === id &&
          recent.chapterId === first.id &&
          Number(recent.scrollTop) > 0
        ) {
    ...
    saveQueueRef.current = saveQueueRef.current
      ...
      .then(async () => {
        ...
        if (currentIdRef.current === chId && editRevisionRef.current === revision) {
          setChapters((prevChs) =>
            prevChs.map((c) =>
              c.id === chId ? { ...c, content: html, wordCount: words, updatedAt: now } : c
            )
          )
          setChapter((prev) =>
            prev && prev.id === chId ? { ...prev, content: html, wordCount: words } : prev
          )
      ...
      .catch((error) => {
        ...
        toast(
          error?.message ||
            'MoonScribe could not save this change locally. Your draft is still open — try Save again.'
        )
      })
    return saveQueueRef.current
  }, [toast])
  ...
  // Leave a recovery copy before the browser hides or unloads the writing
  // surface. The normal debounced save can then finish when the page returns.
  useEffect(() => {
    const flushRecovery = () => {
  ...
  const closeSession = useCallback(async () => {
    ...
    const pausedMs =
      sessionPausedMsRef.current + (sessionPaused ? now - sessionPausedAtRef.current : 0)
  ...
  const resetSession = useCallback((wordCount) => {
    ...
    sessionAutoPausedRef.current = false
  ...
  useEffect(() => {
    const start = () => {
      if (!chapter) return
      resetSession(wordCount)
      setFocusMode(true)
      toast('Writing session started.')
    }
    window.addEventListener('moonscribe:writing-session-start', start)
    return () => window.removeEventListener('moonscribe:writing-session-start', start)
  }, [chapter, resetSession, toast, wordCount, setFocusMode])
  ...
  const handleReport = useCallback(
    (html, words) => {
      ...
      setWordCount(words)
      if (novelRef.current?.sharedRole && canEditSharedNovel) {
        liveSessionRef.current?.publish({ html })
      }
      clearTimeout(liveEditTimer.current)
      liveEditTimer.current = setTimeout(() => {
        const current = chaptersRef.current.find((item) => item.id === currentIdRef.current)
        if (!current || novelRef.current.sharedRole || !canEditSharedNovel) return
  ...
  const selectChapter = useCallback(
    async (chOrId) => {
      ...
      const ch =
        typeof chOrId === 'string' ? chaptersRef.current.find((c) => c.id === chId) : chOrId
      if (!ch) return
      ...
      updateNovel(
        novelRef.current.id,
        { lastChapterId: chId, lastOpened: Date.now() },
        { sync: false }
      )
      saveRecentWriting({
        novelId: novelRef.current.id,
        chapterId: chId,
        mode: activeSection,
        scrollTop: 0,
      })
  ...
  useEffect(() => {
    ...
    const remember = () =>
      saveRecentWriting({
        novelId: novelRef.current.id,
        chapterId: currentIdRef.current,
        mode: activeSection,
  ...
  const handleMobileTouchEnd = useCallback(
    (event) => {
      ...
      const writable = chaptersRef.current.filter(
        (item) => item.kind === 'chapter' || item.kind === 'subchapter'
      )
  ...
  const closeChapterTab = useCallback(
    async (tabId) => {
      ...
      if (tabId === currentIdRef.current && remaining.length)
        await selectChapter(remaining[remaining.length - 1])
      if (!remaining.length) {
        setSplitOpen(false)
      }
    },
  ...
  const workspacePanes = useMemo(
    () =>
      workspacePaneIds.map((paneId) => chapters.find((item) => item.id === paneId)).filter(Boolean),
    [chapters, workspacePaneIds]
  )

  const collaboratorsByChapter = useMemo(
    () =>
      collaboratorPresence.reduce((acc, person) => {
  ...
  const promoteWorkspacePane = useCallback(
    async (chapterId) => {
      if (!chapterId || chapterId === currentIdRef.current) return
      const previousMainId = currentIdRef.current
      await selectChapter(chapterId)
      setWorkspacePaneIds((current) => {
        const withoutPromoted = current.filter(
          (item) => item !== chapterId && item !== previousMainId
        )
  ...
  const toggleSplitView = useCallback(() => {
    ...
    const fallback =
      openChapterTabs.find((tabId) => tabId !== currentIdRef.current) ||
      chapters.find((item) => item.id !== currentIdRef.current)?.id ||
      null
    if (!fallback) {
  ...
  const patchChapterLocal = useCallback((chapterId, patch) => {
    ...
    setChapters((prev) =>
      prev.map((item) => (item.id === chapterId ? { ...item, ...patch } : item))
    )
  ...
  const onComment = useCallback((quote, annotationId = null) => {
    setCommentDraft({ quote: quote || '', comment: '', type: 'note', annotationId })
  ...
  const saveComment = useCallback(async () => {
    ...
    if (novel?.sharedRole && liveDocRef.current) {
      const authorId = String(syncUsername || 'collaborator')
      upsertYjsThread(liveDocRef.current, {
        id: commentDraft.annotationId || `comment-${Date.now()}`,
        chapterId: chapter.id,
        quote: commentDraft.quote,
        createdAt: Date.now(),
        authorId,
        authorName: syncUsername || 'Collaborator',
        resolved: false,
        comments: [
          {
            id: `reply-${Date.now()}`,
            authorId,
            authorName: syncUsername || 'Collaborator',
            text: commentDraft.comment.trim(),
            createdAt: Date.now(),
          },
        ],
      })
      setCommentDraft(null)
      toast('Comment shared with collaborators.')
      return
    }
    ...
    toast(
      novel?.sharedRole
        ? 'Proofread comment saved to the shared novel.'
        : 'Comment kept — private to you.'
    )
  }, [commentDraft, chapter, id, novel?.sharedRole, syncUsername, toast])

  const resolveAnnotation = useCallback(
    async (a) => {
      if (novel?.sharedRole && liveDocRef.current) {
        toggleYjsThreadResolved(
          liveDocRef.current,
          a.id,
          !a.resolved,
          String(syncUsername || 'collaborator')
        )
        return
      }
    ...
    [id, chapter?.id, novel?.sharedRole, syncUsername]
  )

  const removeAnnotation = useCallback(
    async (a) => {
      if (novel?.sharedRole && liveDocRef.current) {
        deleteYjsThread(liveDocRef.current, liveEditorRef.current, a.id)
        return
      }
      await deleteAnnotation(a.id)
      if (chapter?.id) setAnnotations(await listAnnotations(id, chapter.id))
    },
    [id, chapter?.id, novel?.sharedRole]
  )

  const applyLiveRecord = useCallback(
    async (record) => {
      ...
      if (record.store === 'chapters') {
        ...
        if (!incoming || typeof incoming !== 'object' || typeof incoming.content !== 'string')
          return
        ...
        if (
          String(incoming.id) === String(currentIdRef.current) &&
        ...
        if (String(incoming.id) === String(currentIdRef.current)) {
          ...
          setChapter((prev) =>
            prev && String(prev.id) === String(incoming.id) ? { ...prev, ...incoming } : incoming
          )
        ...
        window.dispatchEvent(
          new CustomEvent('moonscribe:shared-media-refresh', { detail: { novelId: id, record } })
        )
      }
    },
    [id, chapter?.id, dirty]
  ...
  // Scene metadata is kept local to each field so typing does not re-render the
  // complete novel workspace or the live document editor.
  const commitMetaField = useCallback(
    (field, value) => {
      commitMeta({ ...(chapter?.meta || EMPTY_META), [field]: value })
    },
    [chapter?.meta, commitMeta]
  )
  ...
  const addNode = useCallback(
    async (kind, parentId = null) => {
      if (kind === 'folder') {
        const rootOrders = [
          ...chapters
            ...
            .map((chapter) => Number(chapter.order) || 0),
          ...folders
        ...
        await createFolder(id, {
          name: 'New folder',
          order: (rootOrders.length ? Math.max(...rootOrders) : 0) + 1,
        })
      ...
      const folderParent =
        parentId && folders.some((folder) => folder.id === parentId) ? parentId : null
  ...
  const handleTrashChapter = useCallback(async () => {
    ...
    toast('Moved to the Trash — recoverable for 30 days.', {
      label: 'Undo',
      run: async () => {
        await restoreTrashed('chapters', target.id)
        const restored = await listChapters(id)
        setChapters(restored)
        if (!chapter && restored[0]) await selectChapter(restored[0].id)
      },
    })
    emitSound('action.destructive')
  ...
  const saveChapterEdit = useCallback(async () => {
    ...
    await updateChapter(editChapter.id, {
      ...
      folderId: ['book', 'part', 'act'].includes(editChapter.kind)
        ? null
        : editChapter.folderId || null,
      status: editChapter.status,
      meta: editChapter.meta || {},
  ...
  const saveMilestone = useCallback(
    async (label) => {
      ...
      const versions = [
        ...(ch.versions || []),
        { at: Date.now(), words, html, label: label || 'Milestone' },
      ]
  ...
  // ---- tidy + merge ----
  const applyTidiedChapter = useCallback(
    (res, chId) => {
      if (!res) return
      setChapters((prev) =>
        prev.map((c) => (c.id === chId && res.chapter ? { ...c, ...res.chapter } : c))
      )
      ...
      toast(
        res.changed
          ? `Tidied — ${bits.join(', ') || 'polish applied'}.`
          : 'Already clean — nothing to tidy.'
      )
    },
    [toast]
  )
  ...
  const applyEditorDesign = useCallback(
    (dId) => {
      ...
      if (dId === 'custom') {
        ...
        persistDesign({
          editorDesign: 'custom',
          customPageBg: customDesignBg,
          customPageText: customDesignText,
        })
  ...
  const importChapters = useCallback(
    async (imported, fileName) => {
      ...
      toast(
        `${imported.length} ${imported.length === 1 ? 'chapter' : 'chapters'} imported from “${fileName}”.`
      )
    },
  ...
  const handleMoveFolder = useCallback(
    async (folderId, targetId, position = 'inside') => {
      if (targetId == null) {
      ...
      const siblings = folders
        .filter(
          (folder) =>
            (folder.parentId || null) === (target.parentId || null) && folder.id !== folderId
        )
        .sort((a, b) => (a.order || 0) - (b.order || 0))
      const index =
        position === 'after'
          ? siblings.findIndex((folder) => folder.id === targetId) + 1
          : Math.max(
              0,
              siblings.findIndex((folder) => folder.id === targetId)
            )
      await moveFolder(
        folderId,
        position === 'inside' ? targetId : target.parentId || null,
        position === 'inside' ? null : index
      )
      await refreshFolders()
      toast('Folder moved.')
    },
  ...
  const handleFolderAppearance = useCallback(
    async (folder, kind) => {
      ...
      await refreshFolders()
      toast(
        `${kind === 'color' ? 'Folder colour' : kind === 'icon' ? 'Folder icon' : 'Folder theme'} updated.`
      )
    },
  ...
  const handleImportFile = useCallback(
    async (file) => {
      ...
      try {
        ...
        toast(error instanceof Error ? error.message : `Could not import “${name}”.`)
      }
    },
    [importChapters, toast]
  )
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      if (!sessionPaused && Date.now() - sessionLastActivityRef.current >= 60_000) {
        sessionPausedAtRef.current = Date.now()
        sessionAutoPausedRef.current = true
        setSessionPaused(true)
      }
      setSessionTick((tick) => tick + 1)
    }, 1000)
    return () => window.clearInterval(timer)
  }, [activeSection, chapter?.id, sessionPaused])

  useEffect(() => {
    sessionLastActivityRef.current = Date.now()
    if (sessionPaused && sessionAutoPausedRef.current) {
      sessionPausedMsRef.current += Math.max(0, Date.now() - sessionPausedAtRef.current)
      sessionPausedAtRef.current = 0
      sessionAutoPausedRef.current = false
      setSessionPaused(false)
    }
    // The pause flag is intentionally read from the render that accompanied a
    // word-count change; adding it here would reset an automatic pause.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [wordCount])
  ...
  const sessionElapsed = Math.max(
    ...
    Date.now() -
      ...
      (sessionPaused ? Date.now() - sessionPausedAtRef.current : 0)
  )
  void sessionTick
  ...
  const sessionWpm =
    sessionElapsed > 30000 ? Math.round(sessionWords / (sessionElapsed / 60000)) : null
  ...
  const mentionsMap = useMemo(
    () => autoChapterMentions(chapters, characters),
    [chapters, characters]
  )
  const openSection = useCallback(
    (seg) => {
      navigate(`/novel/${id}/${seg}`)
    },
    [id, navigate]
  )
      ...
      >
        <div style={{ textAlign: 'center', maxWidth: 420 }}>
          ...
          <p className="muted" style={{ margin: '0 0 28px' }}>
            This novel may have been tucked away, or the shelf was rearranged.
          </p>
  ...
  if (!novel) {
    return (
      <div
        className="app"
        style={{ padding: 'var(--space-7)', textAlign: 'center', color: 'var(--grey)' }}
      >
        Finding your novel…
      </div>
    )
  ...
  return (
    <div
      ...
      style={
        editorDesign === 'custom'
          ? ({
              ['--design-custom-bg' as any]: customDesignBg,
              ['--design-custom-text' as any]: customDesignText,
            } as CSSProperties)
          : undefined
      }
    >
      <Sidebar
        ...
        onFolderSettings={async (folder) => {
          await updateFolder(folder.id, {
            name: folder.name.trim() || 'New folder',
            icon: folder.icon || null,
            color: folder.color || null,
            theme: folder.theme || 'plain',
          })
          await refreshFolders()
        ...
        onMediaDelete={async (file) => {
          await deleteTile(file.id)
      ...
      <div
        ...
        {...(activeSection === 'write'
          ? {
              onDragOver: (e) => {
                if (
                  e.dataTransfer.types.includes(DESIGN_MIME) ||
                  e.dataTransfer.types.includes('text/plain')
                ) {
                  e.preventDefault()
                  e.dataTransfer.dropEffect = e.dataTransfer.types.includes(DESIGN_MIME)
                    ? 'copy'
                    : 'move'
                }
              },
      ...
      >
        <div className="workspace-topbar">
          <div className="actions-row">
            <button
              type="button"
              className="button button-quiet mobile-menu-btn"
              onClick={(event) => {
                event.preventDefault()
                event.stopPropagation()
                setSidebarOpen(true)
              }}
              title="Open chapter library"
              aria-label="Open chapter library"
            >
              <Icon icon="fa-solid fa-book-open" />
              <span className="tablet-library-label">Library</span>
            </button>
            <span className="crumbs">
              ...
              {activeSection === 'write' ? (
                ...
                <>
                  <Link to={`/novel/${id}`}>{novel.title}</Link> ·{' '}
                  <strong>{SECTION_LABELS[activeSection] || activeSection}</strong>
                </>
              )}
            </span>
            <button
              ...
              aria-label="Open novel hub"
            >
              <Icon icon="fa-solid fa-table-cells-large" />
            </button>
          </div>
          {ownerAway && novel?.sharedRole && (
            <div className="collaboration-room-notice" role="status">
              <Icon icon="fa-solid fa-eye" /> Owner is away. You are viewing the last synchronized
              copy read-only; editing resumes when the owner returns.
            </div>
          )}
          <div className="actions-row">
            {canShare && (
              <CollaborationPresence
                ...
                workspace={SECTION_LABELS[activeSection] || activeSection}
                onPresenceChange={setCollaboratorPresence}
                onRecord={applyLiveRecord}
              />
            )}
            <button
              className={`button button-ghost ${!canShare ? 'beta-locked-button' : ''}`}
              onClick={() => canShare && setShareOpen(true)}
              disabled={!canShare}
              title={
                canShare ? 'Invite writers and manage access' : 'Share is locked during the beta'
              }
            >
              <Icon
                icon={canShare ? 'fa-solid fa-user-plus' : 'fa-solid fa-lock'}
                style={{ marginRight: 6 }}
              />{' '}
              Share
            </button>
            {activeSection === 'write' ? (
              reading ? (
                <button
                  className="button button-primary workspace-read-toggle"
                  onClick={() => setReading(false)}
                >
                  <Icon icon="fa-solid fa-pen-nib" style={{ marginRight: 6 }} /> Write
                </button>
              ) : (
                <button
                  className="button button-ghost workspace-read-toggle"
              ...
              <Link className="button button-primary" to={`/novel/${id}`}>
                <Icon icon="fa-solid fa-pen-nib" style={{ marginRight: 6 }} /> Write
              </Link>
            )}
            ...
            <button
              ...
              title={focusMode ? 'Exit focus mode' : 'Focus mode — hide everything but the page'}
              onClick={() => {
                emitSound(focusMode ? 'ui.close' : 'focus.enter')
                setFocusMode(!focusMode)
              }}
            >
              <Icon
                icon={focusMode ? 'fa-solid fa-expand' : 'fa-solid fa-compress'}
                style={{ marginRight: 6 }}
              />{' '}
            ...
            {activeSection === 'write' && (
              <button
                className={`button button-ghost ${referencesOpen ? 'active' : ''}`}
                onClick={() => setReferencesOpen((open) => !open)}
                title="Open story references"
              >
                <Icon icon="fa-solid fa-book-open" style={{ marginRight: 6 }} /> References
              </button>
            )}
            <NotificationBell />
        ...
        {isMobileEditorSurface && (
          <div className="mobile-chapter-header">
            <button
              type="button"
              className="min-h-11 min-w-11"
              onClick={() => navigate(`/novel/${id}`, { state: { openChapters: true } })}
              aria-label="Back to chapters"
              title="Back to chapters"
            >
              <Icon icon="fa-solid fa-arrow-left" />
            </button>
            <span className="mobile-chapter-brand" aria-hidden="true">
              <img src="/moonscribelogo.png" alt="" />
            </span>
            <div>
              <strong>{chapter ? titleFor(chapter, computeNumbers(chapters)) : 'Chapters'}</strong>
              <small>{novel.title}</small>
            </div>
            <button
              type="button"
              className="mobile-chapter-app-menu"
              onClick={() => setMobileHubOpen(true)}
            ...
            <button
              ...
              onClick={(event) =>
                openContextMenu(event, [
                  {
                    label: reading ? 'Edit chapter' : 'Preview chapter',
                    icon: reading ? 'fa-solid fa-pen' : 'fa-regular fa-eye',
                    onClick: () => setReading(!reading),
                  },
                  {
                    label: 'Open Designer',
                    icon: 'fa-solid fa-palette',
                    onClick: () => navigate(`/novel/${id}/design`),
                  },
                ])
              }
        ...
        {activeSection === 'write' && openChapterTabs.length > 0 && (
          <div
            className="chapter-tabbar flex min-w-0 items-center gap-1 overflow-x-auto overscroll-contain px-2 py-1"
          ...
          >
            {openChapterTabs.map((tabId) => {
              ...
              return (
                <button
                  key={tabId}
                  className={`chapter-work-tab ${tabId === currentIdRef.current ? 'active' : ''} ${inWorkspace ? 'secondary' : ''} ${tabPeople.length ? 'has-presence' : ''}`}
                  onClick={() => selectChapter(tabId)}
                  onContextMenu={(event) =>
                    openContextMenu(event, [
                      {
                        label: 'Open chapter',
                      ...
                      {
                        label: inWorkspace ? 'Remove from split view' : 'Open in split view',
                        icon: 'fa-solid fa-table-columns',
                        disabled: tabId === currentIdRef.current,
                        onClick: () =>
                          inWorkspace ? removeWorkspacePane(tabId) : openInSplit(tabId),
                      },
                      {
                        ...
                        onClick: () =>
                          navigator.clipboard?.writeText(
                            `${window.location.origin}/novel/${id}?chapter=${tabId}`
                          ),
                      },
                      'divider',
                      {
                        ...
                        onClick: () => closeChapterTab(tabId),
                      },
                      {
                        label: 'Close other tabs',
                        icon: 'fa-solid fa-table-columns',
                        disabled: openChapterTabs.length < 2,
                        onClick: () => {
                          setOpenChapterTabs([tabId])
                          setWorkspacePaneIds((current) => current.filter((item) => item === tabId))
                        },
                      },
                    ])
                ...
                >
                  ...
                  <span>{tabChapter.title || 'Untitled chapter'}</span>
                  {tabPeople.length > 0 && (
                    <span
                      className="chapter-tab-presence"
                      aria-label={`${tabPeople.length} collaborator${tabPeople.length === 1 ? '' : 's'} in this chapter`}
                      title={tabPeople
                        .map(
                          (person) =>
                            `${person.username} · ${person.activity === 'writing' ? 'writing' : 'viewing'} · ${person.tabName || 'This chapter'}`
                        )
                        .join('\n')}
                    >
                      {tabPeople.slice(0, 3).map((person) => (
                        <span
                          key={person.id}
                          className={`chapter-tab-presence-dot ${person.activity === 'writing' ? 'is-writing' : 'is-viewing'}`}
                          style={
                            {
                              ['--presence-color' as any]: presenceColor(person.id),
                            } as CSSProperties
                          }
                        />
                      ))}
            ...
            <button
              className={`button button-quiet chapter-split-toggle ${splitOpen ? 'active' : ''}`}
            ...
            <span className="chapter-tab-limit">
              {openChapterTabs.length}/{MAX_OPEN_TABS}
            </span>
          </div>
        )}
        ...
        <Suspense fallback={<div className="workspace-loading">Loading workspace…</div>}>
          {activeSection === 'planning' ? (
            <div className="mode-body">
              <PlanningCockpit novelId={id} embedded />
            </div>
          ) : activeSection === 'files' ? (
            <div className="mode-body">
              <ProjectFiles novelId={id} embedded />
            </div>
            ...
            <div className="mode-body story-memory-mode">
              <StoryMemory novelId={id} embedded />
            </div>
          ) : activeSection === 'prose-tools' ? (
            <div className="mode-body prose-tools-mode">
              <ProseTools novelId={id} embedded />
            </div>
            ...
            <div className="mode-body">
              ...
              {activeSection === 'versions' && (
                <ArchiveHub
                  novelId={id}
                  chapters={chapters}
                  onBranchRestored={async () => {
                    const refreshed = await listChapters(id)
                    setChapters(refreshed)
                    if (chapter?.id)
                      setChapter(
                        refreshed.find((item) => item.id === chapter.id) || refreshed[0] || null
                      )
                    toast('Manuscript branch restored.')
                  }}
                  onOpenHistory={async (chapterId) => {
                    await selectChapter(chapterId)
                    setHistoryOpen(true)
                  }}
                />
            ...
            <div className="page">
              <div className="empty">
                ...
                <button className="button button-primary" onClick={() => addChapter('')}>
              ...
              <div className="editor-canvas" style={{ maxWidth: 'none' }}>
                <div className="editor-head">
                  <div
                    ...
                    style={{ borderBottom: 'none', fontSize: '2rem' }}
                  >
                    {titleFor(chapter, computeNumbers(chapters))}
                  </div>
                </div>
                {hasProse ? (
                  <ProsePreview
            ...
            <>
              <div className="scene-meta" aria-label="Scene context">
                ...
                <SceneField
                  label="POV"
                  value={chapter.meta?.pov}
                  options={characters.map((character) => character.name)}
                  onCommit={(value) => commitMetaField('pov', value)}
                />
                <SceneField
                  label="Where"
                  value={chapter.meta?.location}
                  placeholder="The lighthouse"
                  onCommit={(value) => commitMetaField('location', value)}
                />
                <SceneField
                  label="Time"
                  value={chapter.meta?.timeOfDay}
                  options={TIME_OF_DAY}
                  onCommit={(value) => commitMetaField('timeOfDay', value)}
                />
                <SceneField
                  label="Tone"
                  value={chapter.meta?.tone}
                  placeholder="Hushed"
                  onCommit={(value) => commitMetaField('tone', value)}
                />
                <SceneField
                  label="Beat"
                  value={chapter.meta?.beat}
                  placeholder="The letter is opened"
                  onCommit={(value) => commitMetaField('beat', value)}
                />
              </div>

              <div
                className={`editor-host ${splitOpen && workspacePanes.length ? 'editor-host-split' : ''} ${referencesOpen && !isMobileViewport ? 'editor-host-with-references' : ''}`}
              >
                <div
                  className={`split-view ${!isMobileViewport && splitOpen && workspacePanes.length ? 'active' : ''}`}
                  style={
                    !isMobileViewport && splitOpen && workspacePanes.length
                      ? ({ ['--pane-count' as any]: 1 + workspacePanes.length } as CSSProperties)
                      : undefined
                  }
                >
                  <div
                      ...
                      openContextMenu(e, [
                        {
                          ...
                          onClick: () => {
                            setCommentDraft({ quote: '', comment: '', type: 'note' })
                            setAnnotationsOpen(true)
                          },
                        },
                        {
                          label: 'Add milestone snapshot',
                          icon: 'fa-solid fa-flag',
                          onClick: () => saveMilestone('Manual snapshot'),
                        },
                        'divider',
                        {
                          label: 'Open another chapter in split view',
                          icon: 'fa-solid fa-table-columns',
                          onClick: () => toggleSplitView(),
                        },
                        {
                  ...
                  >
                    <Editor
                      ...
                      onReady={(controller) => {
                        liveEditorRef.current = controller
                      }}
                      onComment={onComment}
                      annotations={annotations}
                      typewriterMode={settings.typewriterMode}
                      onSave={saveNow}
                      onDesigns={() => setDesignsOpen((open) => !open)}
                      characters={characters}
                      terms={terms}
                      entities={entities}
                  ...
                  {!isMobileViewport &&
                    splitOpen &&
                    workspacePanes.map((paneChapter, paneIndex) => (
                ...
                {referencesOpen && !isMobileViewport && (
                  <ReferencePane novelId={id} chapterId={chapter?.id} />
                )}
              </div>

              <div className="editor-footer">
                <div className="editor-footer-metrics">
                  <span className="stat editor-footer-primary">
                    <b>{formatWords(wordCount)}</b> words
                  </span>
                  <span className="stat">
                    +<b>{formatWords(sessionWords)}</b> this session
                  </span>
                  <span className="stat" title="Elapsed writing session">
                    <Icon icon="fa-regular fa-clock" /> <b>{sessionClock}</b>
                  </span>
                  <button
                    className="button button-quiet editor-footer-pause"
                    onClick={toggleSessionPause}
                    title={sessionPaused ? 'Resume writing session' : 'Pause writing session'}
                  >
                    <Icon icon={sessionPaused ? 'fa-solid fa-play' : 'fa-solid fa-pause'} />{' '}
                    {sessionPaused ? 'Resume' : 'Pause'}
                  </button>
                  {wordCount > 0 && (
                    <span
                      className="stat"
                      title={`~${lineSpacingWpp} words per page at current spacing`}
                    >
                      ~<b>{Math.ceil(wordCount / lineSpacingWpp)}</b>{' '}
                      {Math.ceil(wordCount / lineSpacingWpp) === 1 ? 'page' : 'pages'}
                    </span>
                  )}
                  {sessionWpm !== null && (
                    <span className="stat">
                  ...
                  {goalWords > 0 ? (
                    <span className="goal-track" title="Daily goal">
                      <span className="bar">
                        <span
                          style={{ width: `${goalPct}%` }}
                          className={goalPct >= 100 ? 'done' : ''}
                        />
                      </span>
                      <span className="stat">
                        <b>{formatWords(todayW)}</b>/{formatWords(goalWords)} today
                      </span>
                    </span>
                  ) : (
                    <span className="stat goal-disabled">Daily goal off</span>
                  )}
                  {!editingGoal ? (
                    <button
                    ...
                    <span className="actions-row">
                      <Select
                        value={String(goalWords)}
                        onChange={(value) => setGoalWords(Number(value))}
                        options={[
                          ...GOAL_PRESETS.map((g) => ({ value: String(g), label: `${g} words` })),
                          { value: '0', label: 'no goal' },
                        ]}
                      />
                      <button className="button button-quiet" onClick={commitGoal}>
                ...
                <div className="editor-footer-actions">
                  <span className="saved-indicator">
                    {dirty ? (
                      ...
                      <>
                        <span className="dot" />{' '}
                        <span role="status" aria-live="polite">
                          Saved locally {savedAt ? timeAgo(savedAt) : ''}
                        </span>
                      </>
                    )}
                  </span>
                  ...
                  {chapter.versions?.length > 0 && (
                    <button className="button button-quiet" onClick={() => setHistoryOpen(true)}>
                      History ({chapter.versions.length})
                    </button>
                  )}
                  <button
                    className="button button-quiet"
                    onClick={() => setDesignsOpen((o) => !o)}
                    title="Premade manuscript designs"
                  >
                    <Icon icon="fa-solid fa-palette" /> <span>Designs</span>
                  </button>
                  <button
                    className="button button-quiet"
                    onClick={() => setReplayOpen((o) => !o)}
                    title="Writing Time Machine — scrub through what you typed this session"
                  >
                    <Icon icon="fa-solid fa-clock-rotate-left" /> <span>Replay</span>
                  </button>
                  <button
                    className="button button-quiet"
                    onClick={() => {
                      setCommentDraft(null)
                      setAnnotationsOpen((o) => !o)
                    }}
                    title="Private comments on this chapter"
                  >
                    <Icon icon="fa-regular fa-comment" />{' '}
                    <span>
                      Comments
                  ...
                  <button
                    className="button button-quiet"
                    onClick={() => setMergeSource(chapter)}
                    title="Merge this chapter with another"
                  >
                    <Icon icon="fa-solid fa-object-ungroup" /> <span>Merge</span>
                  </button>
                </div>
              </div>
                ...
                <>
                  ...
                  <div className="editor-designs" onClick={(e) => e.stopPropagation()}>
                    ...
                    >
                      ...
                      <button className="button button-quiet" onClick={() => setDesignsOpen(false)}>
                        <Icon icon="fa-solid fa-xmark" />
                      </button>
                    </div>
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          ...
                          <input
                            type="color"
                            value={customDesignBg}
                            className="tb-custom-color"
                            onChange={(e) => {
                              const next = e.target.value
                              setCustomDesignBg(next)
                              updateNovel(id, {
                                layout: {
                                  ...(novelRef.current?.layout || {}),
                                  editorDesign: 'custom',
                                  customPageBg: next,
                                  customPageText: customDesignText,
                                },
                        ...
                        <button
                          ...
                          onClick={() => {
                            applyEditorDesign('custom')
                            toast('Custom colours applied.')
                          }}
                        >
                          Apply
                        </button>
                        {editorDesign && (
                          <button
                            ...
                            onClick={() => {
                              ...
                              updateNovel(id, {
                                layout: { ...(novelRef.current?.layout || {}), editorDesign: null },
                              })
                              setDesignsOpen(false)
      ...
      <nav
        className="mobile-novel-nav flex items-end justify-around gap-1 border-t border-white/10 bg-[#0b0b0f]/95 px-2 py-2 pb-[calc(0.5rem+env(safe-area-inset-bottom))] backdrop-blur-xl"
        aria-label="Mobile novel navigation"
      >
        <button
          aria-current={location.pathname === `/novel/${id}` && !libraryOpen ? 'page' : undefined}
          className="grid min-h-11 min-w-11 place-items-center gap-1 rounded-lg px-2 py-1 text-[.62rem] text-[#858188] transition-colors hover:text-[#c79b53] focus-visible:outline-2 focus-visible:outline-[#c79b53]"
          type="button"
        ...
        <button
          type="button"
          className="mobile-novel-create grid h-12 w-12 min-w-12 place-items-center rounded-full border-2 border-[#c79b53] bg-[#141218] text-[#c79b53] shadow-[0_5px_20px_rgba(0,0,0,.45)] focus-visible:outline-2 focus-visible:outline-[#f1d28a]"
          onClick={() => void addNode('chapter')}
          aria-label="Create chapter"
        >
          <Icon icon="fa-solid fa-plus" />
        </button>
        <button
          aria-current={location.pathname.endsWith('/writing-journal') ? 'page' : undefined}
          className="grid min-h-11 min-w-11 place-items-center gap-1 rounded-lg px-2 py-1 text-[.62rem] text-[#858188] transition-colors hover:text-[#c79b53] focus-visible:outline-2 focus-visible:outline-[#c79b53]"
          type="button"
          onClick={() => navigate(`/novel/${id}/writing-journal`)}
        >
      ...
      <ConfirmDialog
        ...
        onConfirm={handleTrashChapter}
        title="Move this chapter to the Trash?"
      >
        “{deleteChapterTarget?.title || 'Untitled'}” will wait in the Trash for 30 days. Its words
        stay with you — restore it any time.
      </ConfirmDialog>
...
function MobileNovelHub({ novel, onClose, onOpenLibrary, onExport, onSettings }) {
      ...
      if (event.key === 'Tab') {
        const focusable = Array.from(
          document.querySelectorAll<HTMLElement>(
            '.mobile-hub-sheet button, .mobile-hub-sheet a, .mobile-hub-sheet input, .mobile-hub-sheet select, .mobile-hub-sheet textarea'
          )
        ).filter(
          (element) =>
  ...
  const groups: Array<[string, Array<[string, string, string]>]> = [
    [
      'WRITE',
      [
        ['Chapters', '', 'fa-solid fa-list'],
        ['Draft board', 'corkboard', 'fa-solid fa-table-columns'],
      ],
    ],
    [
      'PLAN',
      [
        ['Planning cockpit', 'planning', 'fa-solid fa-diagram-project'],
        ['Characters', 'characters', 'fa-solid fa-user-group'],
        ['Worldbuilding', 'world', 'fa-solid fa-globe'],
        ['Relationships', 'relationships', 'fa-solid fa-share-nodes'],
        ['Timeline / continuity', 'timeline', 'fa-solid fa-timeline'],
      ],
    ],
    [
      'CREATE',
      [
        ['Designer', 'design', 'fa-solid fa-palette'],
        ['Media library', 'media', 'fa-solid fa-images'],
      ],
    ],
    [
      'REVIEW',
      [
        ['Story memory', 'story-memory', 'fa-solid fa-brain'],
        ['Prose tools', 'prose-tools', 'fa-solid fa-wand-magic-sparkles'],
  ...
  const go = (path) => {
    onClose()
    if (path === 'settings') onSettings?.()
    else if (path === 'help') navigate('/dashboard?view=insights')
    else if (path === 'export') onExport()
    else if (path) navigate(`/novel/${novel.id}/${path}`)
    else onOpenLibrary()
  }
  return (
    <div
      className="mobile-hub-overlay fixed inset-0 z-[2400] flex items-end bg-black/65 backdrop-blur-sm"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div className="mobile-hub-sheet w-full max-h-[92dvh] touch-pan-y overscroll-contain overflow-y-auto rounded-t-3xl border-t border-[#c79b53]/30 bg-[#0b0b0f] px-4 pb-[calc(5.5rem+env(safe-area-inset-bottom))] pt-2 shadow-[0_-22px_70px_rgba(0,0,0,.5)]">
        ...
        <header>
          <button ref={closeRef} type="button" onClick={onClose} aria-label="Close novel hub">
            <Icon icon="fa-solid fa-xmark" />
          </button>
          <div>
            <span>ACTIVE NOVEL</span>
            <h2 id="mobile-hub-title">{novel.title}</h2>
            <p>
              <i className="mobile-hub-online-dot" aria-label="Synced online" />
              Your story workspace
            </p>
          </div>
        </header>
        <label className="mobile-hub-search flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.025] px-3">
        ...
        <div className="mobile-hub-groups grid gap-5">
          {groups.map(([label, items]) => {
            const filtered = items.filter(([title]) =>
              title.toLowerCase().includes(query.toLowerCase().trim())
            )
            return filtered.length ? (
              <section className="grid gap-1" key={label}>
                <span className="mobile-hub-label mb-1 text-[.62rem] font-bold tracking-[.16em] text-[#c79b53]">
                  {label}
                </span>
                {filtered.map(([title, path, icon]) => (
                  <button
                    className="flex min-h-11 items-center gap-3 rounded-xl border border-white/10 bg-white/[.025] px-3 text-left text-sm text-[#eee8df] transition-colors hover:border-[#c79b53]/40 hover:bg-[#c79b53]/10 focus-visible:outline-2 focus-visible:outline-[#c79b53]"
                    type="button"
                    key={title}
                    onClick={() => go(path)}
                  >
...
function ChapterEditModal({ chapter, onChange, onClose, onSave }) {
  ...
  return (
    <Modal
      ...
      width={680}
      className="folder-settings-modal"
    >
      {chapter && (
        <>
          <nav className="folder-settings-tabs" aria-label="Chapter settings sections">
            {[
              ['overview', 'Overview', 'fa-solid fa-feather-pointed'],
              ['appearance', 'Appearance', 'fa-solid fa-palette'],
              ['details', 'Details', 'fa-solid fa-sliders'],
          ...
          <div className="folder-settings-panel">
            {tab === 'overview' && (
              <>
                <div className="folder-settings-intro">
                  <span>MANUSCRIPT NODE</span>
                  <strong>Shape how this item appears in your book.</strong>
                </div>
                <div className="field">
                  <label>
                    Title <span className="hint">(blank uses the derived name)</span>
                  </label>
                  <input
                    value={chapter.title || ''}
                ...
                <div className="field">
                  <label>Kind</label>
                  <Select
                    ariaLabel="Chapter kind"
                    width="100%"
                    value={chapter.kind || 'chapter'}
                    onChange={(value) => onChange({ ...chapter, kind: value })}
                    options={kinds.map(([value, label]) => ({ value, label }))}
                  />
                  <p className="small muted">
                    Books, parts and acts become outline headers; chapters are numbered, and
                    subchapters nest beneath chapters.
                  </p>
                </div>
              </>
            )}
                  ...
                  <>
                    <div className="field">
                      ...
                      <Select
                        ...
                        width="100%"
                        value={chapter.icon || ''}
                        onChange={(value) => onChange({ ...chapter, icon: value || null })}
                        options={[
                          ['', 'Automatic'],
                          ...
                          ['fa-solid fa-feather-pointed', 'Writing'],
                    ...
                    >
                      <label style={{ flex: 1 }}>
                        ...
                        <input
                          ...
                          type="color"
                          value={chapter.color || '#c79b53'}
                          onChange={(e) => onChange({ ...chapter, color: e.target.value })}
                        />
                      </label>
                      <label style={{ flex: 1 }}>
                        Theme
                        <Select
                          ariaLabel="Folder theme"
                          width="100%"
                          value={chapter.folderTheme || 'plain'}
                          onChange={(value) => onChange({ ...chapter, folderTheme: value })}
                          options={[
            ...
            {tab === 'details' && (
              <>
                <div className="field">
                  ...
                  <input
                    value={chapter.part || ''}
                    onChange={(e) => onChange({ ...chapter, part: e.target.value })}
                    placeholder="Legacy part label — mostly unused now"
                  />
                </div>
                <div className="field">
                  <label>Status</label>
                  <Select
                    ...
                    onChange={(value) => onChange({ ...chapter, status: value })}
                    options={[
                ...
                <div className="chapter-metadata-grid">
                  {[
                    ['pov', 'POV character', 'Who owns this scene?'],
                    ['location', 'Location', 'Where does it happen?'],
                    ['timeOfDay', 'Time', 'Dawn, evening, year 3…'],
                    ['tone', 'Tone', 'Quiet, tense, celebratory…'],
                    ['beat', 'Story beat', 'What changes in this scene?'],
                    ['sceneGoal', 'Scene goal', 'What does someone want?'],
                    ['conflict', 'Conflict', 'What gets in the way?'],
                    ['outcome', 'Outcome', 'What is different by the end?'],
                  ].map(([key, label, placeholder]) => (
                    <label className="field" key={key}>
                      <span>{label}</span>
                      <input
                        value={chapter.meta?.[key] || ''}
                        placeholder={placeholder}
                        onChange={(event) =>
                          onChange({
                            ...chapter,
                            meta: { ...(chapter.meta || {}), [key]: event.target.value },
                          })
                        }
                      />
                    </label>
                ...
                <div className="chapter-revision-passes">
                  <span className="field-label">Revision passes</span>
                  <p className="small muted">Track the kind of work this chapter still needs.</p>
                  <div className="revision-pass-grid">
                    {[
                      ...
                      const checked = !!chapter.meta?.revisionPasses?.[key]
                      return (
                        <label key={key}>
                          <input
                            type="checkbox"
                ...
                <p className="small muted">
                  Scene metadata stays with the chapter and can be used by planning, timeline, and
                  continuity tools.
                </p>
              </>
            )}
    ...
    <div className="split-editor split-editor-secondary">
      <div className="split-pane-topbar">
        ...
        <div className="split-pane-actions">
          <button
            className="button button-quiet"
            onClick={onPromote}
            title="Make this the main chapter"
          >
      ...
      <div className="split-pane-footer">
        ...
        <span className="saved-indicator">
          {dirty ? (
            <>
              <span className="dot" style={{ background: 'var(--rose)' }} /> saving…
            </>
          ) : (
            <>
              <span className="dot" /> saved {timeAgo(savedAt)}
            </>
          )}
        </span>
    ...
    <Modal open={open} onClose={onClose} title="Version history" width={compare ? 760 : 480}>
      {compare ? (
        <>
          ...
          <SentenceComparison
            before={htmlToPlain(compare.html)}
            after={currentText}
            beforeLabel={compare.label ? compare.label : timeAgo(compare.at)}
            beforeWords={compare.words}
            afterWords={countWords(chapter?.content || '')}
          />
          <div className="modal-foot">
            <button className="button button-ghost" onClick={() => setCompare(null)}>
              Cancel
            </button>
            <button className="button button-primary" onClick={() => onRestore(compare)}>
              ...
              {[...chapter.versions].reverse().map((v, i) => (
                <div key={v.at + i} className={`version-row ${v.label ? 'milestone' : ''}`}>
                  ...
                  <span style={{ flex: 1, color: 'var(--charcoal)', fontSize: '0.88rem' }}>
                    {v.label && (
                      <b className="version-label">
                        <Icon icon="fa-solid fa-flag" /> {v.label}
                      </b>
                    )}
...
function SentenceComparison({ before, after, beforeLabel, beforeWords, afterWords }) {
  ...
  return (
    <>
      <div className="version-diff-summary" aria-live="polite">
        <span>
          <i className="diff-dot removed" /> {removed} removed
        </span>
        <span>
          <i className="diff-dot added" /> {added} added
        </span>
        <small>Sentence-level comparison</small>
      </div>
      <div className="version-compare">
        <div className="version-col">
          <div className="version-col-head">
            This version{' '}
            <span>
              {beforeLabel} · {formatWords(beforeWords)} w
            </span>
          </div>
          <div className="version-col-body sentence-diff">
            {changes
              .filter((item) => item.type !== 'added')
              .map((item, index) => (
                <span key={`${item.type}-${index}`} className={`diff-sentence ${item.type}`}>
                  {item.text}
                </span>
              ))}
          </div>
        ...
        <div className="version-col">
          <div className="version-col-head">
            Now <span>{formatWords(afterWords)} w</span>
          </div>
          <div className="version-col-body sentence-diff">
            {changes
