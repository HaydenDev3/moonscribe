...
export default function Landing() {
  ...
  return <main className="landing">
    ...
    <section className="landing-hero">
      ...
      <div className="landing-actions">{cloudOnly || lockedDownload ? <button className={`button button-primary landing-primary ${lockedDownload ? 'beta-locked-button' : ''}`} onClick={signIn}><Icon icon={lockedDownload ? 'fa-solid fa-lock' : 'fa-solid fa-cloud'} /> {lockedDownload ? 'Desktop beta access' : platform === 'mobile' ? 'Write in MoonScribe Cloud' : 'Open MoonScribe Cloud'}</button> : <a className="button button-primary landing-primary" href={downloadUrl} download><Icon icon="fa-solid fa-download" /> {downloadLabel}</a>}<button className="button button-secondary" onClick={signIn}>{cloudOnly || lockedDownload ? 'Sign in' : 'Use Cloud instead'}</button><a className="button button-secondary" href="#studio"><Icon icon="fa-solid fa-play" /> See the studio</a></div>
      <p className="landing-platform-note">{platform === 'mobile' ? 'Phones and tablets use MoonScribe Cloud—nothing to install.' : downloadUrl ? `MoonScribe detected ${platformLabel(platform)}. Cloud remains available in any modern browser.` : `A ${platformLabel(platform)} desktop build is not published yet. MoonScribe Cloud is ready now.`}</p>
