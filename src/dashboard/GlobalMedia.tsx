...
export default function GlobalMedia({ novels, onOpenNovel }) {
  ...
  return <section className="global-media-page" onClick={() => setMenu(null)}>
    ...
    {loading ? <div className="global-media-empty"><Icon icon="fa-solid fa-spinner" /><h2>Gathering your images…</h2></div> : !visible.length ? <div className="global-media-empty"><Icon icon="fa-regular fa-images" /><h2>{items.length ? 'No matching media' : 'Your visual library is empty'}</h2><p>{items.length ? 'Try a different search.' : 'Add images here or inside any novel’s Media Library.'}</p></div> : <div className="global-media-grid">{visible.map((item) => <article className="global-media-card" key={`${item.novelId}-${item.id}`} onContextMenu={(event) => { event.preventDefault(); setMenu({ x: event.clientX, y: event.clientY, item }) }} onDoubleClick={() => setPreview(item)}><img src={item.image} alt={item.text || 'Media reference'} /><span><strong>{item.text || 'Untitled image'}</strong><small>{item.novelTitle} · {(dataUrlBytes(item.image) / 1024).toFixed(0)} KB</small></span></article>)}</div>}
    {menu && <div className="media-context-menu global-media-context" style={{ left: menu.x, top: menu.y }} onClick={(event) => event.stopPropagation()}><button onClick={() => { setPreview(menu.item); setMenu(null) }}><Icon icon="fa-solid fa-eye" /> Preview</button><button onClick={() => onOpenNovel(menu.item.novelId)}><Icon icon="fa-solid fa-folder-open" /> Manage novel</button><button onClick={() => rename(menu.item)}><Icon icon="fa-solid fa-pen" /> Rename</button><button className="danger" onClick={async () => { await deleteTile(menu.item.id); setItems((current) => current.filter((item) => item.id !== menu.item.id)); setMenu(null); toast('Image deleted.') }}><Icon icon="fa-solid fa-trash" /> Delete</button></div>}
