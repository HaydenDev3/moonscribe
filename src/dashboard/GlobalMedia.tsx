...
export default function GlobalMedia({ novels, onOpenNovel }) {
  ...
  return <section className="global-media-page" onClick={() => setMenu(null)}>
    ...
    <div className="global-media-toolbar"><label><Icon icon="fa-solid fa-magnifying-glass" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search images and novels…" /></label><Select ariaLabel="Sort media" value={sort} onChange={setSort} options={[{ value: 'recent', label: 'Recently added' }, { value: 'name', label: 'Name' }, { value: 'size', label: 'File size' }]} /><label className="global-media-upload"><Icon icon="fa-solid fa-arrow-up-from-bracket" /> Add images<input hidden type="file" accept="image/*" multiple onChange={(event) => { uploadFiles(Array.from(event.target.files || [])); event.target.value = '' }} /></label></div>
