...
function createBook(book: BookRecord, index: number) {
  ...
  const backMap = imageMap(book.coverDesign?.backImage); const backMaterial = new THREE.MeshStandardMaterial({ color: book.coverDesign?.backColor || color, roughness: .85 }); if (backMap) backMaterial.map = backMap; const back = new THREE.Mesh(board, backMaterial); back.rotation.y = Math.PI; back.position.z = -thickness / 2 - .045; root.add(back)
  ...
  root.rotation.y = Math.PI / 2; root.position.set(0, 0, (index % 3 - 1) * .025); const bounds = new THREE.Box3().setFromObject(root); const size = bounds.getSize(new THREE.Vector3()); root.userData.spineWidth = size.x; root.userData.bookDepth = size.z; root.userData.baseX = 0; root.userData.baseY = root.position.y; root.userData.baseZ = root.position.z; root.userData.scaleTarget = new THREE.Vector3(1, 1, 1)
  return root
}
