...
export default function Sidebar({
  ...
  const [navCollapsed, setNavCollapsed] = useState(
    () => new Set(['World', 'Craft', 'Journal', 'Archive'])
  )
  ...
  const [chapterSort, setChapterSort] = useState<'order' | 'number' | 'alpha'>(
    () => novel?.layout?.chapterSort || 'order'
  )
  ...
  useEffect(() => {
    if (novel?.id) getWorkspacePreferences(novel.id).then(setWorkspacePrefs)
  }, [novel?.id])
  ...
  const visibleChapters = chapters.filter((c) => {
    ...
    const text = `${c.title || ''} ${c.kind || ''} ${c.content || ''}`.toLowerCase()
  ...
  const sortedVisibleChapters = useMemo(() => {
    ...
    next.sort((a, b) => {
      if (chapterSort === 'alpha')
        return collator.compare(a.title || titleFor(a, numbers), b.title || titleFor(b, numbers))
  ...
  const searchResults = useMemo(() => {
    ...
    chapters
      .filter((chapter) =>
        `${chapter.title || ''} ${chapter.kind || ''} ${chapter.content || ''}`
          .toLowerCase()
          .includes(needle)
      )
      .slice(0, 8)
      .forEach((chapter) =>
        results.push({
          type: 'Chapter',
          label: chapter.title || titleFor(chapter, numbers),
          detail: 'Manuscript',
    ...
    NOVEL_NAV.flatMap((group) => group.items)
      .filter((item) => `${item.label} ${item.to}`.toLowerCase().includes(needle))
      .slice(0, 8)
      .forEach((item) =>
        results.push({
          type: 'Workspace',
          ...
          icon: item.icon,
          action: () => {
            navigate(itemPath(novel.id, item))
            setQuery('')
          },
        })
      )
    mediaFiles
      .filter((file) => `${file.text || ''} media image file`.toLowerCase().includes(needle))
      .slice(0, 6)
      .forEach((file) =>
        results.push({
          type: 'Media',
          label: file.text || 'Untitled image',
  ...
  }, [chapters, mediaFiles, navigate, novel.id, numbers, onMediaSelect, onSelect, query])
      ...
      canEdit
        ? [
            { label: 'Edit', icon: 'fa-solid fa-pen', onClick: () => onEdit(c) },
            {
              ...
              icon: c.favorite ? 'fa-solid fa-star' : 'fa-regular fa-star',
            ...
            },
            {
              ...
              disabled: i === -1 || i >= flat.length - 1,
              onClick: () => onMove(c.id, 1),
            },
            'divider',
            {
              label: 'Add chapter here',
              icon: 'fa-solid fa-file-plus',
              onClick: () =>
                onAdd('chapter', isContainer(c) ? c.id : c.folderId || c.parentId || null),
            },
            ...(c.kind === 'chapter'
              ? [
                  {
      ...
      canEdit
        ? [
            ...
            },
            {
              ...
              onClick: () => onAdd('prologue', null),
            },
            {
              label: 'New epilogue',
              icon: 'fa-solid fa-feather-pointed',
              onClick: () => onAdd('epilogue', null),
            },
            {
              label: 'New folder',
              icon: 'fa-solid fa-folder-plus',
              onClick: () => onAdd('folder', null),
            },
            {
              label: 'New act',
              icon: 'fa-solid fa-layer-group',
              onClick: () => onAdd('act', null),
            },
            'divider',
            {
              label: 'Open chapter library',
              icon: 'fa-solid fa-table-list',
              onClick: onOpenLibrary,
            },
            {
              label: manuscriptCollapsed ? 'Expand manuscript' : 'Collapse manuscript',
  ...
  const navGroupMenu = (e, group) => {
    openContextMenu(e, [
      {
        label: navCollapsed.has(group.group) ? `Expand ${group.group}` : `Collapse ${group.group}`,
      ...
      ...group.items.map((item) => ({
        ...
        icon: item.icon,
        onClick: () => navigate(itemPath(novel.id, item)),
      })),
    ])
  }
  ...
  const handleDropOn = (e, target) => {
    ...
    if (isContainer(target) && ratio >= 0.25 && ratio <= 0.75) {
      ...
      !isContainer(target) &&
      isContainer(chapters.find((chapter) => chapter.id === dragId))
    ) {
  ...
  const renderNode = (node, depth) => {
    ...
    return (
      <div key={c.id} className="binder-node">
        <div
          className={`binder-item ${c.id === currentId ? (location.pathname.endsWith(`/novel/${novel.id}`) ? 'active' : 'recent') : ''} ${container ? `is-container folder-theme-${c.folderTheme || 'plain'}` : ''} ${dropTarget === c.id ? 'drop-target' : ''}`}
          style={
            {
              paddingLeft: 12 + depth * 14,
              ...(folderColor ? { ['--folder-color' as any]: folderColor } : {}),
            } as CSSProperties
          }
          onClick={() => {
            onSelect(c.id)
            if (!location.pathname.endsWith(`/novel/${novel.id}`)) navigate(`/novel/${novel.id}`)
          }}
          onKeyDown={(e) => {
        ...
        >
          ...
          {/* Title */}
          <Icon
            icon={container ? folderIcon : 'fa-regular fa-file-lines'}
            className="binder-node-icon"
            style={folderColor ? { color: folderColor } : undefined}
          />
          <span className="binder-label">{label}</span>
          {c.favorite && (
            <Icon icon="fa-solid fa-star binder-favorite" aria-label="Favorite chapter" />
          )}
          {tabPeople.length > 0 && (
            <span
            ...
            >
              {tabPeople.slice(0, 3).map((person) => (
                <span
                  ...
                  style={
                    {
                      ['--presence-color' as any]:
                        person.activity === 'writing' ? 'var(--accent)' : 'var(--panel-ink)',
                    } as CSSProperties
                  }
          ...
          {/* Word count */}
          {!container && c.wordCount > 0 && <span className="binder-words">{c.wordCount}</span>}
  ...
  const renderFolder = (folder, depth = 0) => {
    ...
    const childChapters = chapters.filter(
      (chapter) =>
        chapter.folderId === folder.id &&
        (!query.trim() ||
          `${chapter.title || ''} ${chapter.kind || ''}`
            .toLowerCase()
    ...
    return (
      <div className="binder-node" key={folder.id}>
        <div
          ...
          onContextMenu={(event) => {
            ...
            openContextMenu(event, [
              {
                label: 'New chapter inside folder',
                icon: 'fa-solid fa-file-circle-plus',
                onClick: () => onAdd('chapter', folder.id),
              },
              {
                label: 'Folder settings',
                icon: 'fa-solid fa-sliders',
                onClick: () => {
                  setFolderSettingsTab('overview')
          ...
          onDragLeave={() =>
            setDropTarget((current) => (current === `folder:${folder.id}` ? null : current))
          }
          onDragEnter={(event) => {
            event.preventDefault()
            event.stopPropagation()
          ...
          onDrop={(event) => {
            ...
            if (payload?.startsWith('folder:')) {
              const sourceId = payload.slice(7)
            ...
            } else if (payload?.startsWith('chapter:'))
              onMoveToFolder?.(payload.slice(8), folder.id)
            setDraggingId(null)
            setDropTarget(null)
          }}
          draggable
          onDragStart={(event) => {
            ...
            event.dataTransfer.setData('application/x-moonscribe-item', `folder:${folder.id}`)
            event.dataTransfer.effectAllowed = 'move'
            setDraggingId(`folder:${folder.id}`)
          }}
          onDragEnd={() => {
            setDraggingId(null)
            setDropTarget(null)
        ...
        >
          <span className={`binder-caret${hasChildren ? '' : ' empty'}`}>
            <Icon
              icon={
                hasChildren
                  ? isCollapsed
        ...
        {hasChildren && !isCollapsed && (
          <div className="binder-children">
            {children.map((child) =>
              child.type === 'folder'
                ? renderFolder(child.value, depth + 1)
                : renderNode({ ch: child.value, children: [] }, depth + 1)
            )}
          </div>
        )}
      </div>
  ...
  return (
    <aside className={`sidebar ${open ? 'open' : ''}`}>
      ...
      <div className="sidebar-head">
        <div className="sidebar-brand-banner" aria-hidden="true">
        ...
        <div className="sidebar-head-top group relative isolate overflow-hidden rounded-xl border border-white/10 bg-white/[0.035] shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_8px_24px_rgba(0,0,0,0.12)] backdrop-blur-xl transition-all duration-300 ease-out hover:border-white/15 hover:bg-white/[0.055] focus-within:-translate-y-px focus-within:border-[color:var(--accent)]/50 focus-within:bg-white/[0.07] focus-within:shadow-[0_0_0_3px_color-mix(in_srgb,var(--accent)_12%,transparent),0_12px_30px_rgba(0,0,0,0.2)]">
          <span
            aria-hidden="true"
            className="sidebar-title-sheen pointer-events-none absolute inset-y-0 -left-1/2 z-0 w-1/3 -skew-x-12 bg-gradient-to-r from-transparent via-white/[0.08] to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100 group-focus-within:opacity-100"
          />
          <input
        ...
        <div className="sidebar-novel-meta">
          ...
          <span>
            {chapters.filter((c) => !isContainer(c)).length}{' '}
            {chapters.filter((c) => !isContainer(c)).length === 1 ? 'chapter' : 'chapters'}
          </span>
        </div>
      </div>
      ...
      >
        ...
        <div className="binder-section">
          {!manuscriptCollapsed && (
            <div className="sidebar-search sidebar-search-top">
              <Icon icon="fa-solid fa-magnifying-glass" />
              <input
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="Search around this novel…"
                aria-label="Search around this novel"
              />
              {query && (
                <button
                  type="button"
              ...
              {!!searchResults.length && (
                <div
                  className="novel-search-results"
                  role="listbox"
                  aria-label="Novel search results"
                >
          ...
          <div className="binder-section-header" onContextMenu={manuscriptMenu}>
            ...
            >
              <Icon
                icon={manuscriptCollapsed ? 'fa-solid fa-caret-right' : 'fa-solid fa-caret-down'}
                className="binder-section-caret"
              />
            ...
            <div className="binder-section-actions">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  void updateNovel(novel.id, {
                    layout: { ...(novel.layout || {}), chapterSort: value },
                  })
                }}
              >
                <Icon
          ...
          {!manuscriptCollapsed && (
            <>
              {/* Search is kept above the section header for quick access. */}
              {/* Chapters and folders are separate trees. A folder is a filing
                  container, never an outline chapter or Act. */}
              {!!folders.length && (
                <div className="binder-folders-section binder-folders-primary">
                  <div className="binder-section-header binder-folders-header">
                    <span className="binder-section-label">
                      <Icon icon="fa-solid fa-folder-tree" /> Folders
                    </span>
                    <span className="binder-folder-count">{folders.length}</span>
                  </div>
                  <div className="binder-tree binder-folder-tree">
                    {folders
              ...
              {chapters.length === 0 ? (
                <p className="binder-empty">
                  {folders.length ? 'No loose chapters —' : 'No chapters yet —'}
                  <br />
                  the first one is waiting.
                </p>
              ) : (
                <div className="binder-tree">{tree.map((node) => renderNode(node, 0))}</div>
              )}
              {draggingId && (
                <button
                  type="button"
                  ...
                  onDrop={(event) => {
                    event.preventDefault()
                    event.stopPropagation()
                    const dragId = readDragPayload(event)
                    if (dragId?.startsWith('folder:')) onMoveFolder?.(dragId.slice(7), null, 'root')
                    else if (dragId?.startsWith('chapter:')) onMoveToFolder?.(dragId.slice(8), null)
                    else if (dragId) onReorder(dragId, null, null)
                    setDraggingId(null)
                    setDropTarget(null)
                  }}
                >
              ...
              <div className="binder-node binder-media-folder">
                <button
                  className="binder-item binder-media-link"
                  type="button"
                  onClick={() => setMediaCollapsed((value) => !value)}
                  aria-expanded={!mediaCollapsed}
                >
                  <Icon
                    icon={mediaCollapsed ? 'fa-solid fa-caret-right' : 'fa-solid fa-caret-down'}
                  />
                  <Icon icon="fa-regular fa-images" />
                  <span>Media</span>
                  <small>{mediaFiles.length} files</small>
                </button>
                {!mediaCollapsed && (
                  <div className="binder-children binder-media-files">
                    {(showAllMedia ? mediaFiles : mediaFiles.slice(0, 5)).map((file) => (
                      <button
                        className="binder-item binder-media-file"
                        key={file.id}
                        type="button"
                        ...
                        onContextMenu={(event) => {
                          ...
                          openContextMenu(event, [
                            ...
                            },
                            {
                              ...
                              icon: 'fa-solid fa-trash',
                              danger: true,
                              onClick: () => onMediaDelete?.(file),
                            },
                          ])
                        }}
                      >
                        <img src={file.image} alt="" className="binder-media-thumb" />
                        <span className="binder-label">{file.text || 'Untitled image'}</span>
                      </button>
                    ))}
                    {mediaFiles.length > 5 && (
                      <button
                        type="button"
                        className="binder-item binder-media-more"
                        onClick={() => setShowAllMedia((value) => !value)}
                      >
                        <Icon
                          icon={showAllMedia ? 'fa-solid fa-chevron-up' : 'fa-solid fa-ellipsis'}
                        />
                        <span>
                          {showAllMedia
                            ? 'Show fewer files'
                    ...
                    {!mediaFiles.length && (
                      <button
                        className="binder-media-file binder-media-empty"
                        type="button"
                        onClick={onMediaSelect}
                      >
                        <Icon icon="fa-solid fa-plus" />
        ...
        <nav className="nav-groups" aria-label="Novel sections">
          <div className="workspace-manager-link">
            <button
              type="button"
              className="nav-item"
            ...
            >
              ...
              <span className="nav-item-label">Customize workspaces</span>
            </button>
          </div>
          {NOVEL_NAV.map((g) => ({
            ...g,
            ...
            .map((g) => {
              ...
              return (
                <div key={g.group} className={`nav-group${isGroupCollapsed ? ' collapsed' : ''}`}>
                  <button
                      ...
                      (() => {
                        ...
                        return (
                          <NavLink
                            ...
                            className={({ isActive }) =>
                              `nav-item${isActive ? ' active' : ''}${designerLive ? ' has-live-presence' : ''}`
                            }
                            onClick={onClose}
                            onContextMenu={(e) => {
                              e.preventDefault()
                              openContextMenu(e, [
                                {
                                  label: `Open ${n.label}`,
                                  icon: n.icon,
                                  onClick: () => navigate(itemPath(novel.id, n)),
                                },
                                'divider',
                                {
                                  ...
                                  onClick: () => {
                                    updateSettings({
                                      hiddenSidebarTabs: [
                                        ...new Set([...(settings.hiddenSidebarTabs || []), n.to]),
                                      ],
                                    })
                          ...
                          >
                            ...
                            {designerLive && (
                              <span
                                className="nav-item-live-indicator"
                                aria-hidden="true"
                                title={
                                  designerLive
                                    ? 'Collaborator active in Designer'
                                    : 'Collaborator active in this chapter'
                                }
                              />
                            )}
                          </NavLink>
      ...
      {workspaceManagerOpen && (
        ...
        >
          <p className="workspace-manager-intro">
            Shape this novel’s studio around the way you work. Choose the spaces you want in the
            sidebar; Manuscript is always available.
          </p>
          <div className="workspace-manager-summary">
            <span>
              <Icon icon="fa-solid fa-layer-group" /> {(workspacePrefs?.enabled || []).length} of{' '}
              {WORKSPACE_REGISTRY.length} workspaces visible
            </span>
            <span>
          ...
          <div className="workspace-manager-list">
            {WORKSPACE_REGISTRY.map((item) => {
              const enabled =
                item.key === 'write' || (workspacePrefs?.enabled || []).includes(item.key)
              return (
                <label
                  key={item.key}
                  className={`workspace-manager-row${enabled ? ' is-enabled' : ''}${item.required ? ' is-required' : ''}`}
                >
                  <span className="workspace-manager-icon">
                    <Icon icon={item.icon} />
                  </span>
                  <span className="workspace-manager-copy">
                    <strong>{item.label}</strong>
                    <small>{item.required ? 'Always available' : `${item.group} workspace`}</small>
                  </span>
                  ...
                  <input
                    ...
                    onChange={() => void toggleWorkspace(item.key)}
                    aria-label={`${enabled ? 'Hide' : 'Show'} ${item.label}`}
                  />
                </label>
              )
            })}
          </div>
          <div className="modal-foot workspace-manager-foot">
            <button
              className="button button-ghost"
              onClick={async () => {
                const next = await resetWorkspacePreferences(novel.id)
                setWorkspacePrefs(next)
              }}
            >
              <Icon icon="fa-solid fa-rotate-left" /> Restore defaults
            </button>
            <button
        ...
        <div className="sidebar-user-info">
          <div
            ...
            style={
              profileBanner
                ? {
                    backgroundImage: `linear-gradient(90deg, rgba(25,20,16,.9), rgba(25,20,16,.62)), url(${profileBanner})`,
                    backgroundSize: 'cover',
                    backgroundPosition: 'center',
                  }
                : undefined
            }
            onContextMenu={openAccountProfile}
          >
            <button
            ...
            >
              <span className="dashboard-account-avatar">
                <ProfileAvatar
                  src={profileAvatar || syncDiscordAvatar}
                  name={syncUsername || 'MoonScribe writer'}
                />
                <i
                  className={`dashboard-account-presence sync-dot-${syncStatus || 'offline'}`}
                  aria-label={`Sync status: ${syncStatus || 'offline'}`}
                />
              </span>
            ...
            <div className="dashboard-account-actions dashboard-sidebar-label">
              <button type="button" onClick={onSyncClick} aria-label="Sync now" title="Sync now">
          ...
          <ProfilePopover
            ...
            profile={{
              ...
              displayName: settings?.displayName || syncUsername || 'Local writer',
              writerName: settings?.writerName,
              bio: settings?.profileBio,
              avatarUrl: profileAvatar || syncDiscordAvatar,
              ...
              activity:
                syncStatus === 'syncing'
                  ...
                  : syncStatus === 'local'
                    ...
                    : null,
              showActivity: false,
              role: settings?.role || 'Writer',
              joinedAt: null,
            }}
            actions={[
              { label: 'Profile preview', icon: 'fa-solid fa-user', onClick: openSettings },
                ...
                ? [
                    {
                      label: 'Copy username',
                      icon: 'fa-regular fa-copy',
                      onClick: () => navigator.clipboard?.writeText(syncUsername),
                    },
                  ]
                : []),
              {
                ...
                onClick: async () => {
                  ...
                  toast(syncUsername ? 'Signed out.' : 'Offline mode closed.')
                },
              },
            ]}
          />
        ...
        {folderSettings && (
          <div className="folder-settings-panel">
            <nav className="folder-settings-tabs" aria-label="Folder settings sections">
              <button
                type="button"
                className={folderSettingsTab === 'overview' ? 'active' : ''}
                onClick={() => setFolderSettingsTab('overview')}
              >
              ...
              <button
                type="button"
                className={folderSettingsTab === 'appearance' ? 'active' : ''}
                onClick={() => setFolderSettingsTab('appearance')}
              >
              ...
              <button
                type="button"
                className={folderSettingsTab === 'details' ? 'active' : ''}
                onClick={() => setFolderSettingsTab('details')}
              >
                <Icon icon="fa-solid fa-sliders" />
                Details
              </button>
            </nav>
            {folderSettingsTab === 'overview' && (
              <>
                <div className="folder-settings-intro">
                  <span>MANUSCRIPT FOLDER</span>
                  <strong>Shape how this folder appears in your manuscript.</strong>
                </div>
                <label className="field">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setFolderSettings({ ...folderSettings, name: event.target.value })
                    }
                  />
                </label>
                <div className="field">
                  <span>Folder preview</span>
                  <div className="folder-settings-preview">
                    <Icon
                      icon={folderSettings.icon || 'fa-solid fa-folder'}
                      style={{ color: folderSettings.color || 'var(--accent)' }}
                    />
                    <strong>{folderSettings.name || 'New folder'}</strong>
                    <small>Organizational folder</small>
                  </div>
                </div>
              </>
            )}
            {folderSettingsTab === 'appearance' && (
              <>
                <div className="folder-settings-intro">
                  ...
                  <strong>Give this folder its own visual character.</strong>
                </div>
                <label className="field">
                  <span>Icon</span>
                  <Select
                    value={folderSettings.icon || 'fa-solid fa-folder'}
                    onChange={(value) => setFolderSettings({ ...folderSettings, icon: value })}
                    options={[
                      { value: 'fa-solid fa-folder', label: 'Folder' },
                      { value: 'fa-solid fa-folder-open', label: 'Open folder' },
                      { value: 'fa-solid fa-star', label: 'Starred' },
                      { value: 'fa-solid fa-moon', label: 'Moon' },
                      { value: 'fa-solid fa-feather-pointed', label: 'Writing' },
                    ]}
                  />
                </label>
                <div className="field folder-colour-picker">
                  <span>Colour</span>
                  <div className="folder-colour-picker-control">
                    <input
                      aria-label="Choose folder colour"
                      type="color"
                      value={folderSettings.color || '#c9953d'}
                      onChange={(event) =>
                        setFolderSettings({ ...folderSettings, color: event.target.value })
                      }
                    />
                    <output>{folderSettings.color || '#c9953d'}</output>
                  </div>
                </div>
                <label className="field">
                  <span>Theme</span>
                  <Select
                    value={folderSettings.theme || 'plain'}
                    onChange={(value) => setFolderSettings({ ...folderSettings, theme: value })}
                    options={[
            ...
            {folderSettingsTab === 'details' && (
              <>
                <div className="folder-settings-intro">
                  <span>DETAILS</span>
                  <strong>Folder structure and behavior.</strong>
                </div>
                <div className="folder-settings-detail-grid">
                  <div>
                    <span>Contents</span>
                    <strong>
                      {chapters.filter((chapter) => chapter.folderId === folderSettings.id).length}{' '}
                      documents
                    </strong>
                    <small>Chapters and manuscript pages</small>
                  </div>
                  <div>
                    <span>Location</span>
                    <strong>{folderSettings.parentId ? 'Nested folder' : 'Manuscript root'}</strong>
                    <small>
                      {folderSettings.parentId
                        ? 'Inside another folder'
                        : 'Top-level manuscript folder'}
                    </small>
                  </div>
                  <div>
                    <span>Created</span>
                    <strong>
                      {folderSettings.createdAt
                        ? new Date(folderSettings.createdAt).toLocaleDateString()
                        : 'Earlier version'}
                    </strong>
                    <small>Folder record</small>
                  </div>
                  <div>
                    <span>Status</span>
                    <strong>
                      {folderSettings.isExpanded === false ? 'Collapsed' : 'Expanded'}
                    </strong>
                    <small>Sidebar display state</small>
                  </div>
                </div>
              </>
            )}
            <div className="modal-foot">
              ...
              <button
                ...
                onClick={async () => {
                  await onFolderSettings?.(folderSettings)
