...
export default function RolePermissions() {
  ...
  const roleBadges = (accountRoles.length ? accountRoles : ['user']).map((role) => {
    ...
    return (
      <span key={role} className={`settings-status-pill ${meta.tone}`}>
        {meta.label}
      </span>
    )
  })
  ...
  const updateRole = async (userId: string, nextRoles: string[]) => {
    try {
      ...
      setUsers((current) =>
        current.map((user) => (user.id === userId ? { ...user, roles: nextRoles } : user))
      )
    } catch (error) {
  ...
  return (
    <section className="settings-panel">
      ...
      <h2>Permissions</h2>
      <p className="muted">
        MoonScribe roles default to User; Admin and Developer permissions can be granted as needed.
      </p>
        ...
        <div className="settings-section-card" style={{ marginTop: 16 }}>
          ...
          <div style={{ display: 'grid', gap: 10, marginTop: 12 }}>
            {loading ? (
              ...
              <div className="muted">No other users are connected yet.</div>
            ) : (
              users.map((user) => (
                <div
                  key={user.id}
