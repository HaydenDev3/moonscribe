  ...
  return (
    <Modal open={!!source} onClose={onClose} title="Merge chapters" width={480}>
      <p className="small muted" style={{ marginTop: 0 }}>
        Two chapters become one. The absorbed chapter's words join at a clean seam — a scene break
        or a blank line — and nothing is deleted without a second look.
      </p>
      ...
      <div className="field">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <label className="check-row">
            <input
              type="radio"
              checked={direction === 'into-current'}
              onChange={() => setDirection('into-current')}
            />
            Append{' '}
            <b>
              {others.find((c) => c.id === targetId)?.title ||
                (others[0]?.title ?? 'another chapter')}
            </b>{' '}
            into “{source.title || 'Untitled'}”
          </label>
          <label className="check-row">
            <input
              type="radio"
              checked={direction === 'into-other'}
              onChange={() => setDirection('into-other')}
            />
            Append “{source.title || 'Untitled'}” into{' '}
            <b>
              {others.find((c) => c.id === targetId)?.title ||
                (others[0]?.title ?? 'another chapter')}
            </b>
      ...
      <div className="field">
        ...
        <Select
          ...
          options={others.map((c) => ({
            value: c.id,
            label: `${c.title || 'Untitled'} · ${formatWords(c.wordCount || 0)} words`,
          }))}
        />
      </div>
      ...
      <div className="field">
        ...
        <Select
          ariaLabel="Seam between chapters"
          value={separator}
          onChange={setSeparator}
          options={[
            { value: 'scene-break', label: 'Scene break (❦)' },
            { value: 'space', label: 'One blank line' },
          ]}
        />
      </div>
      ...
      {keep && absorb && (
        ...
        >
          “{keep.title || 'Untitled'}” keeps <b>{formatWords(keep.wordCount || 0)}</b> words and
          gains <b>{formatWords(absorb.wordCount || 0)}</b> from “{absorb.title || 'Untitled'}” →{' '}
