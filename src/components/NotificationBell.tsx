...
type Notice = NotificationListItem
...
export default function NotificationBell({ buttonClassName = '' }: { buttonClassName?: string }) {
  ...
  const load = async () => {
    ...
    const response = await fetch(`${config.server.replace(/\/$/, '')}/api/notifications`, {
    ...
    const connect = async () => {
      ...
      socket.onmessage = (event) => {
        try {
          ...
          if (!data.notification.readAt && settings.desktopNotifications !== false)
            void notifyDesktop(
              data.notification.title || 'MoonScribe',
              data.notification.body || 'You have a new notification.'
            )
        } catch {
  ...
  useEffect(() => {
    ...
    window.addEventListener('moonscribe:open-notifications', openNotifications)
    return () => window.removeEventListener('moonscribe:open-notifications', openNotifications)
  }, [])
  const markRead = async (id: string) => {
    const config = await getConfig()
    if (!config.server || !config.token) return
    await fetch(`${config.server.replace(/\/$/, '')}/api/notifications/${id}/read`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${config.token}` },
    })
    setItems((current) =>
  ...
  const markAll = async () => {
    const config = await getConfig()
    if (!config.server || !config.token) return
    await fetch(`${config.server.replace(/\/$/, '')}/api/notifications/read-all`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${config.token}` },
    })
  ...
  return (
    <div className="notification-bell" style={{ position: 'relative' }}>
      <button
        className={`button button-quiet ${buttonClassName}`.trim()}
      ...
      {open && (
        <div className="notification-popover" role="dialog" aria-label="Notifications">
          <div className="notification-popover-header">
            ...
            {unread > 0 && (
              <button className="button button-quiet" onClick={() => void markAll()}>
                Mark all read
              </button>
            )}
          </div>
          <div className="notification-tabs" role="tablist">
            {['All', 'Writing', 'Collaboration', 'Account'].map((tab) => (
              <button
                key={tab}
          ...
          {items.filter(
            ...
            <div className="notification-empty">
              ...
              <p className="muted">
                No new notes from MoonScribe, collaborators or your writing goals.
              </p>
            </div>
          ) : (
            <NotificationList
